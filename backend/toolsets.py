...
_NAABIGA_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on NAABIGA_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (NAABIGA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `naabiga tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``naabiga tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NAABIGA_KANBAN_TASK env "
    ...
    # Coding posture (base Naabiga — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in naabiga_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Naabiga toolsets (CLI + messaging platforms)
    #
    ...
    # the `naabiga send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "naabiga-cli": {
        ...
        "tools": _NAABIGA_CORE_TOOLS,
        "includes": []
    },

    "naabiga-cron": {
        # Mirrors naabiga-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `naabiga tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as naabiga-cli; gated by `naabiga tools`",
        "tools": _NAABIGA_CORE_TOOLS,
        "includes": []
    },
    ...
    "naabiga-email": {
        "description": "Email bot toolset - interact with Naabiga via email (IMAP/SMTP)",
        "tools": _NAABIGA_CORE_TOOLS,
        "includes": []
    },
    ...
    "naabiga-sms": {
        "description": "SMS bot toolset - interact with Naabiga via SMS (Twilio)",
        "tools": _NAABIGA_CORE_TOOLS,
        "includes": []
    },
    ...
    "naabiga-webhook": {
        ...
        "tools": _NAABIGA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "naabiga-gateway": {
        ...
        "tools": [],
        "includes": ["naabiga-telegram", "naabiga-discord", "naabiga-whatsapp", "naabiga-slack", "naabiga-signal", "naabiga-bluebubbles", "naabiga-homeassistant", "naabiga-email", "naabiga-sms", "naabiga-mattermost", "naabiga-matrix", "naabiga-dingtalk", "naabiga-feishu", "naabiga-wecom", "naabiga-wecom-callback", "naabiga-weixin", "naabiga-qqbot", "naabiga-webhook", "naabiga-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``naabiga-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_NAABIGA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``naabiga-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (naabiga-<name>).
        # Gives them _NAABIGA_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("naabiga-"):
            platform_name = name[len("naabiga-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NAABIGA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

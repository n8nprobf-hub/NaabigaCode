...
_THOT_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on THOT_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (THOT_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `thot tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (THOT_KANBAN_TASK env "
    ...
    # Coding posture (base Thot — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in thot_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Thot toolsets (CLI + messaging platforms)
    #
    ...
    # the `thot send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "thot-cli": {
        ...
        "tools": _THOT_CORE_TOOLS,
        "includes": []
    },

    "thot-cron": {
        # Mirrors thot-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `thot tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as thot-cli; gated by `thot tools`",
        "tools": _THOT_CORE_TOOLS,
        "includes": []
    },
    ...
    "thot-email": {
        "description": "Email bot toolset - interact with Thot via email (IMAP/SMTP)",
        "tools": _THOT_CORE_TOOLS,
        "includes": []
    },
    ...
    "thot-sms": {
        "description": "SMS bot toolset - interact with Thot via SMS (Twilio)",
        "tools": _THOT_CORE_TOOLS,
        "includes": []
    },
    ...
    "thot-webhook": {
        ...
        "tools": _THOT_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "thot-gateway": {
        ...
        "tools": [],
        "includes": ["thot-telegram", "thot-discord", "thot-whatsapp", "thot-slack", "thot-signal", "thot-bluebubbles", "thot-homeassistant", "thot-email", "thot-sms", "thot-mattermost", "thot-matrix", "thot-dingtalk", "thot-feishu", "thot-wecom", "thot-wecom-callback", "thot-weixin", "thot-qqbot", "thot-webhook", "thot-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``thot-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_THOT_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``thot-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (thot-<name>).
        # Gives them _THOT_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("thot-"):
            platform_name = name[len("thot-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_THOT_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

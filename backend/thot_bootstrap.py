"""Windows UTF-8 bootstrap for Thot entry points.
...
should be imported at the very top of every Thot entry point
(``thot``, ``thot-agent``, ``thot-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Thot modules.

    Thot ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Thot source root
    ...
    """
    root = src_root or os.environ.get("THOT_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``THOT_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("THOT_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import thot_bootstrap``
# (or ``from thot_bootstrap import apply_windows_utf8_bootstrap``) at

"""Windows UTF-8 bootstrap for Naabiga entry points.
...
should be imported at the very top of every Naabiga entry point
(``naabiga``, ``naabiga-agent``, ``naabiga-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Naabiga modules.

    Naabiga ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Naabiga source root
    ...
    """
    root = src_root or os.environ.get("NAABIGA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``NAABIGA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("NAABIGA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import naabiga_bootstrap``
# (or ``from naabiga_bootstrap import apply_windows_utf8_bootstrap``) at

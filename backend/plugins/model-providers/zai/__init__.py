...
request omits ``thinking``.  Thot' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Thot reasoning
...
(per Z.AI / BigModel docs).  Thot' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Thot reasoning effort onto GLM-5.2's native ``high``/``max``.

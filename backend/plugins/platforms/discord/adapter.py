...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("NAABIGA_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid NAABIGA_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.naabiga/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("NAABIGA_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("NAABIGA_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = env_float(
            "NAABIGA_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "NAABIGA_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Naabiga gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``NAABIGA_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("NAABIGA_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_naabiga_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Naabiga manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "naabiga_voice",
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Naabiga pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="new", description="Démarrer une nouvelle conversation")
        async def slash_new(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/reset", "Nouvelle conversation démarrée~")

        @tree.command(name="reset", description="Réinitialiser votre session Naabiga")
        async def slash_reset(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/reset", "Session réinitialisée~")

        @tree.command(name="model", description="Afficher ou changer le modèle")
        @discord.app_commands.describe(name="Nom du modèle (ex. anthropic/claude-sonnet-4). Laissez vide pour voir le modèle actuel.")
        async def slash_model(interaction: discord.Interaction, name: str = ""):
            await self._run_simple_slash(interaction, f"/model {name}".strip())

        @tree.command(name="reasoning", description="Afficher ou changer l'effort de raisonnement")
        @discord.app_commands.describe(effort="Effort de raisonnement : none, minimal, low, medium, high ou xhigh.")
        async def slash_reasoning(interaction: discord.Interaction, effort: str = ""):
            await self._run_simple_slash(interaction, f"/reasoning {effort}".strip())

        @tree.command(name="personality", description="Définir une personnalité")
        @discord.app_commands.describe(name="Nom de la personnalité. Laissez vide pour lister les disponibles.")
        ...
        @tree.command(name="retry", description="Renvoyer votre dernier message")
        async def slash_retry(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/retry", "Nouvel essai~")

        @tree.command(name="undo", description="Supprimer le dernier échange")
        async def slash_undo(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/undo")

        @tree.command(name="status", description="Afficher l'état de la session Naabiga")
        async def slash_status(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/status", "État envoyé~")

        @tree.command(name="sethome", description="Définir ce chat comme canal d'accueil")
        async def slash_sethome(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/sethome")

        @tree.command(name="stop", description="Arrêter l'agent Naabiga en cours")
        async def slash_stop(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/stop", "Arrêt demandé~")

        @tree.command(name="steer", description="Injecter un message après le prochain appel d'outil (sans interrompre)")
        @discord.app_commands.describe(prompt="Texte à injecter dans le prochain résultat d'outil de l'agent")
        ...
        @tree.command(name="compress", description="Compresser le contexte de conversation")
        async def slash_compress(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/compress")

        @tree.command(name="title", description="Définir ou afficher le titre de la session")
        @discord.app_commands.describe(name="Titre de session. Laissez vide pour afficher l'actuel.")
        ...
        @tree.command(name="resume", description="Reprendre une session précédemment nommée")
        @discord.app_commands.describe(name="Nom de session à reprendre. Laissez vide pour lister les sessions.")
        async def slash_resume(interaction: discord.Interaction, name: str = ""):
            await self._run_simple_slash(interaction, f"/resume {name}".strip())

        @tree.command(name="usage", description="Afficher l'utilisation de jetons de cette session")
        async def slash_usage(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/usage")

        @tree.command(name="help", description="Afficher les commandes disponibles")
        async def slash_help(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/help")

        @tree.command(name="insights", description="Afficher les informations et analyses d'utilisation")
        @discord.app_commands.describe(days="Nombre de jours à analyser (défaut : 7)")
        ...
        @tree.command(name="reload-mcp", description="Recharger les serveurs MCP depuis la configuration")
        async def slash_reload_mcp(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/reload-mcp")

        @tree.command(name="reload-skills", description="Re-scanner ~/.naabiga/skills/ pour les compétences ajoutées ou supprimées")
        async def slash_reload_skills(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/reload-skills")

        @tree.command(name="voice", description="Activer/désactiver le mode de réponse vocale")
        @discord.app_commands.describe(mode="Mode vocal : join, channel, leave, on, tts, off ou status")
        @discord.app_commands.choices(mode=[
            ...
            discord.app_commands.Choice(name="join — rejoindre votre salon vocal", value="join"),
            discord.app_commands.Choice(name="channel — rejoindre votre salon vocal (alias)", value="channel"),
            discord.app_commands.Choice(name="leave — quitter le salon vocal", value="leave"),
            discord.app_commands.Choice(name="on — réponse vocale aux messages vocaux", value="on"),
            discord.app_commands.Choice(name="tts — réponse vocale à tous les messages", value="tts"),
            discord.app_commands.Choice(name="off — texte uniquement", value="off"),
            discord.app_commands.Choice(name="status — afficher le mode actuel", value="status"),
        ])
        async def slash_voice(interaction: discord.Interaction, mode: str = ""):
            await self._run_simple_slash(interaction, f"/voice {mode}".strip())

        @tree.command(name="update", description="Mettre à jour Naabiga Agent vers la dernière version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Mise à jour lancée~")

        @tree.command(name="restart", description="Redémarrer proprement la passerelle Naabiga")
        async def slash_restart(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/restart", "Redémarrage demandé~")

        @tree.command(name="approve", description="Approuver une commande dangereuse en attente")
        @discord.app_commands.describe(scope="Optionnel : 'all', 'session', 'always', 'all session', 'all always'")
        ...
        @tree.command(name="deny", description="Refuser une commande dangereuse en attente")
        @discord.app_commands.describe(scope="Optionnel : 'all' pour refuser toutes les commandes en attente")
        async def slash_deny(interaction: discord.Interaction, scope: str = ""):
            await self._run_simple_slash(interaction, f"/deny {scope}".strip())

        @tree.command(name="thread", description="Créer un nouveau fil et y démarrer une session Naabiga")
        @discord.app_commands.describe(
            name="Nom du fil",
            message="Premier message optionnel à envoyer à Naabiga dans le fil",
            auto_archive_duration="Archivage auto en minutes (60, 1440, 4320, 10080)",
        )
        ...
        @tree.command(name="queue", description="Mettre un prompt en file pour le prochain tour (n'interrompt pas)")
        @discord.app_commands.describe(prompt="Le prompt à mettre en file")
        async def slash_queue(interaction: discord.Interaction, prompt: str):
            await self._run_simple_slash(interaction, f"/queue {prompt}", "Mis en file pour le prochain tour.")

        @tree.command(name="background", description="Exécuter un prompt en arrière-plan")
        @discord.app_commands.describe(prompt="Le prompt à exécuter en arrière-plan")
        async def slash_background(interaction: discord.Interaction, prompt: str):
            await self._run_simple_slash(interaction, f"/background {prompt}", "Tâche d'arrière-plan démarrée~")

        # ── Auto-register any gateway-available commands not yet on the tree ──
        # This ensures new commands added to COMMAND_REGISTRY in
        # naabiga_cli/commands.py automatically appear as Discord slash
        ...
        def _build_auto_slash_command(_name: str, _description: str, _args_hint: str = ""):
            ...
            desc = (_description or f"Exécuter /{_name}")[:100]
        ...
        dropped_over_cap = 0
        try:
            from naabiga_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from naabiga_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Naabiga skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from naabiga_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Naabiga session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Naabiga: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Naabiga has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Naabiga"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Naabiga: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Naabiga semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Naabiga session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Naabiga handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Naabiga to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Naabiga needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Naabiga needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``naabiga update --gateway``
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Naabiga could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_naabiga_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``naabiga pairing approve``).
    ...
    # so users approved via ``naabiga pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``naabiga update`` prompts.
...
# is not in this process (e.g. ``naabiga cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("NAABIGA_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["NAABIGA_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("NAABIGA_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["NAABIGA_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'naabiga-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # naabiga_cli/setup.py::_setup_discord function.  Same shape as Teams.

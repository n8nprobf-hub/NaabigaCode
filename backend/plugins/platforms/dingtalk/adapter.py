...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "Thot", "text": normalized},
        }
    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            out_track_id = f"thot_{uuid.uuid4().hex[:12]}"
...
# dict in thot_cli/gateway.py, and the _send_dingtalk dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces thot_cli/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in thot_cli/gateway.py. CLI helpers are
    ...
    if method == 0:
        try:
            from thot_cli.dingtalk_auth import dingtalk_qr_auth

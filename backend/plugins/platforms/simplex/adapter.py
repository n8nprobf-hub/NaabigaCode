"""SimpleX Chat platform adapter (Naabiga plugin).
...
``plugins/platforms/simplex/``. The Naabiga plugin loader scans the
...
Optional environment variables:
    ...
    NAABIGA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``naabiga setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Naabiga — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("NAABIGA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``naabiga setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.naabiga/.env`` via
    ``naabiga_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "naabiga_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.naabiga/.env"
        )
        return

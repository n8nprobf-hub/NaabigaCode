"""SimpleX Chat platform adapter (Thot plugin).
...
``plugins/platforms/simplex/``. The Thot plugin loader scans the
...
discoverable and ``thot setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Thot — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("THOT_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``thot setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.thot/.env`` via
    ``thot_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "thot_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.thot/.env"
        )
        return

...
# ``model_tools`` import time, so ``naabiga status``, ``naabiga chat``, etc.
...
# ``naabiga_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Naabiga is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a naabiga MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NAABIGA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from naabiga_constants import get_naabiga_home as _get_naabiga_home
            _naabiga_home = _get_naabiga_home()
        except (ModuleNotFoundError, ImportError):
            _naabiga_home = _Path.home() / ".naabiga"
        self._thread_count_store = _ThreadCountStore(
            _naabiga_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Naabiga is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NAABIGA_HOME", str(_Path.home() / ".naabiga"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NAABIGA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a naabiga MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Naabiga is thinking…' marker message.
        ...
        "Naabiga is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Naabiga is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Naabiga is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Naabiga is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Naabiga is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``naabiga setup``.

    The setup wizard at ``naabiga_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.naabiga/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. naabiga-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.naabiga/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'naabiga-agent[google_chat]'",
        ...
        # the "Naabiga is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Naabiga is thinking…' marker message "

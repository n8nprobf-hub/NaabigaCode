...
# ``model_tools`` import time, so ``thot status``, ``thot chat``, etc.
...
# ``thot_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Thot is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a thot MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${THOT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from thot_constants import get_thot_home as _get_thot_home
            _thot_home = _get_thot_home()
        except (ModuleNotFoundError, ImportError):
            _thot_home = _Path.home() / ".thot"
        self._thread_count_store = _ThreadCountStore(
            _thot_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Thot is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("THOT_HOME", str(_Path.home() / ".thot"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${THOT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a thot MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Thot is thinking…' marker message.
        ...
        "Thot is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Thot is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Thot is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Thot is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Thot is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``thot setup``.

    The setup wizard at ``thot_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.thot/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. thot-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.thot/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'thot-agent[google_chat]'",
        ...
        # the "Thot is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Thot is thinking…' marker message "

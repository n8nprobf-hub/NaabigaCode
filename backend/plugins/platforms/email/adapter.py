"""
Email platform adapter for the Naabiga gateway.

Allows users to interact with Naabiga by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "naabiga-agent" "version" "{_naabiga_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `naabiga config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `naabiga gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Naabiga Agent")
        ...
        msg_id = f"<naabiga-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# naabiga_cli/gateway.py, and the _send_email dispatch in

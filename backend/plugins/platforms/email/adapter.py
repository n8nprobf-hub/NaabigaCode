"""
Email platform adapter for the Thot gateway.

Allows users to interact with Thot by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "thot-agent" "version" "{_thot_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `thot config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `thot gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Thot Agent")
        ...
        msg_id = f"<thot-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# thot_cli/gateway.py, and the _send_email dispatch in

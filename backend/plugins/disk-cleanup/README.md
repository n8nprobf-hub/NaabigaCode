...
Auto-tracks and cleans up ephemeral files created during Naabiga Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NAABIGA_HOME` and `/tmp/naabiga-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NAABIGA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under NAABIGA_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `NAABIGA_HOME` or `/tmp/naabiga-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NAABIGA_HOME/disk-cleanup/` is itself excluded
- `$NAABIGA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

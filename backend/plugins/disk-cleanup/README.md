...
Auto-tracks and cleans up ephemeral files created during Thot Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$THOT_HOME` and `/tmp/thot-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `THOT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under THOT_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `THOT_HOME` or `/tmp/thot-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$THOT_HOME/disk-cleanup/` is itself excluded
- `$THOT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

...
``{title, url, description, position}`` rows every other Naabiga web
provider produces.
...
prefers Naabiga-managed xAI Grok OAuth (via ``naabiga auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.naabiga/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``naabiga tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `naabiga auth` to sign in with "

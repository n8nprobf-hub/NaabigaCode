...
``{title, url, description, position}`` rows every other Thot web
provider produces.
...
prefers Thot-managed xAI Grok OAuth (via ``thot auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.thot/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``thot tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `thot auth` to sign in with "

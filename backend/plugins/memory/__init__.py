...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with naabiga-agent)
2. User-installed providers: ``$NAABIGA_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_naabiga_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_naabiga_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_naabiga_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$NAABIGA_HOME/plugins/`` or None if unavailable."""
    try:
        from naabiga_constants import get_naabiga_home
        d = get_naabiga_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:  # noqa: F821 — forward ref
    """Load and return a MemoryProvider instance by name.

    Checks both bundled (``plugins/memory/<name>/``) and user-installed
    (``$NAABIGA_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:  # noqa: F821 — forward ref
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _naabiga_user_memory.<name>.cli, usually

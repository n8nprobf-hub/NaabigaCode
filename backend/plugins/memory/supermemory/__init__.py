...
_DEFAULT_CONTAINER_TAG = "naabiga"
...
def _load_supermemory_config(naabiga_home: str) -> dict:
    config = _default_config()
    config_path = Path(naabiga_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, naabiga_home: str) -> None:
    config_path = Path(naabiga_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "naabiga"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Naabiga writes into the "Naabiga" Space in the Supermemory
        ...
        merged = {"sm_source": "naabiga", **(metadata or {})}
...
def _resolve_container_tag_for_setup(naabiga_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(naabiga_home)
...
def _probe_supermemory_connection(api_key: str, naabiga_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(naabiga_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(naabiga_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `naabiga memory setup`.
        # All other options are documented for $NAABIGA_HOME/supermemory.json
    ...
    def save_config(self, values, naabiga_home):
        ...
        _save_supermemory_config(sanitized, naabiga_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, naabiga_home)
    ...
    def post_setup(self, naabiga_home: str, config: dict) -> None:
        ...
        from naabiga_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(naabiga_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from naabiga_constants import get_naabiga_home
        self._naabiga_home = kwargs.get("naabiga_home") or str(get_naabiga_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._naabiga_home)

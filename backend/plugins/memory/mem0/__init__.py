...
Configuration
-------------
Secret (lives in $NAABIGA_HOME/.env or the environment):
...
Behavioral settings (live in $NAABIGA_HOME/mem0.json, set via `naabiga memory
setup`):
  ...
  agent_id           — Agent identifier (default: naabiga)
...
def _load_config() -> dict:
    """Load config from env vars, with $NAABIGA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "naabiga"),
        "oss": {},
    }
    ...
    config_path = get_naabiga_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, naabiga_home):
        """Write config to $NAABIGA_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, naabiga_home: str, config: dict) -> None:
        ...
        post_setup(naabiga_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $NAABIGA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "naabiga")

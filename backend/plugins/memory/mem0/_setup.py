...
def _save_mem0_json(naabiga_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(naabiga_home) / "mem0.json"
    existing = {}
...
def _setup_platform(naabiga_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "naabiga-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "naabiga"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.naabiga/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.naabiga/.env (or unset it) or Naabiga will keep "
    ...
    provider.save_config(provider_config, naabiga_home)

    if env_writes:
        _write_env(Path(naabiga_home) / ".env", env_writes)
...
def _setup_selfhosted(naabiga_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "naabiga-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "naabiga")
...
def _setup_oss(naabiga_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(naabiga_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "naabiga-user")
    ...
    if env_writes:
        _write_env(Path(naabiga_home) / ".env", env_writes)
    _save_mem0_json(naabiga_home, {"mode": "oss", "user_id": user_id, "agent_id": "naabiga", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, naabiga_home: str) -> str:
...
_PGVECTOR_CONTAINER = "naabiga-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "naabiga"
...
def _setup_oss_interactive(naabiga_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], naabiga_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], naabiga_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'naabiga-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "naabiga-user")

    agent_id = input("  Agent ID [naabiga]: ").strip()
    agent_id = agent_id or "naabiga"
    ...
    if env_writes:
        _write_env(Path(naabiga_home) / ".env", env_writes)
    _save_mem0_json(naabiga_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(naabiga_home: str, config: dict) -> None:
    """Entry point called by naabiga memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(naabiga_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(naabiga_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(naabiga_home, config, flags)
        return

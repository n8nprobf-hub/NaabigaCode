"""`naabiga meet node ...` subcommand tree.

Wired into the existing ``naabiga meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``naabiga meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="naabiga-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``naabiga meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             naabiga meet node approve <name> ws://<host>:{args.port} {token}")
        try:

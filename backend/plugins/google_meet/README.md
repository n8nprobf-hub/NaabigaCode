...
Let the thot agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where thot runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `thot meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `thot meet` CLI |
...
| `cli.py` | `thot meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$THOT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `thot meet node {run,list,approve,remove,status,ping}` |
...
thot meet install                                      # pip + Chromium
thot meet setup                                        # preflight
thot meet auth                                         # optional
thot meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
thot meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.thot/.env
thot meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
thot meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
thot meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.thot/.env
thot meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, thot will **not** switch your system audio input automatically — the
...
thot plugins enable google_meet
thot meet node run --display-name my-mac --host 0.0.0.0 --port 18789

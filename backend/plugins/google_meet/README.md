...
Let the naabiga agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where naabiga runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `naabiga meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `naabiga meet` CLI |
...
| `cli.py` | `naabiga meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$NAABIGA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `naabiga meet node {run,list,approve,remove,status,ping}` |
...
naabiga meet install                                      # pip + Chromium
naabiga meet setup                                        # preflight
naabiga meet auth                                         # optional
naabiga meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
naabiga meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.naabiga/.env
naabiga meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
naabiga meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
naabiga meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.naabiga/.env
naabiga meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, naabiga will **not** switch your system audio input automatically — the
...
naabiga plugins enable google_meet
naabiga meet node run --display-name my-mac --host 0.0.0.0 --port 18789

"""thot hooks — inspect and manage shell-script hooks.

Usage::

    thot hooks list
    thot hooks test <event> [--for-tool X] [--payload-file F]
    thot hooks revoke <command>
    thot hooks doctor

Consent records live under ``~/.thot/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.thot/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``thot hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: thot hooks {list|test|revoke|doctor}")
        print("Run 'thot hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.thot/config.yaml.")
        print("See `thot hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `thot hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `thot hooks test` and `thot hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Thot wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `thot hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `thot hooks doctor` would execute

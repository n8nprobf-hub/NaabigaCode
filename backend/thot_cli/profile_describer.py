...
Used by ``thot profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``thot_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Thot Agent kanban board.
...
Rules:
  ...
  - Never write "Thot Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from thot_constants import get_thot_home  # type: ignore
            profile_dir = Path(get_thot_home())
        else:

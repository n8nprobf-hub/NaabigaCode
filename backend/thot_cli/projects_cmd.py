"""``thot project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$THOT_HOME/projects.db`` store
(see :mod:`thot_cli.projects_db`).
...
from thot_cli import projects_db as pdb
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Thot Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``thot project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: thot project <action> [options]\n"
                "Run 'thot project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `thot project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `thot project add-folder`.",
            file=sys.stderr,
        )
        return 1

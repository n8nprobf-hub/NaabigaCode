...
on Japanese locales, etc.).  Thot's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``thot_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Thot is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``THOT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("THOT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Thot-managed Git install directories so
    ...
    # already-running shells don't see the change — which means thot
    ...
    # install without Thot-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Thot's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Thot's
    ``thot config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Thot (User env var in Windows Settings, or
    export in a PowerShell profile) and Thot picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Thot-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\thot\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``thot`` from right after install) retain their old PATH.

    Any subprocess Thot spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Thot-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "thot", "git", "cmd"),
        os.path.join(local_appdata, "thot", "git", "bin"),
        os.path.join(local_appdata, "thot", "git", "usr", "bin"),
        # Thot venv Scripts directory — host of the thot.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes thot, but harmless to include.
        os.path.join(local_appdata, "thot", "thot-agent", "venv", "Scripts"),

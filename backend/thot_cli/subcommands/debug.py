...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Thot Agent. Use 'thot debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    thot debug share              Upload debug report (asks for confirmation)
    thot debug share --yes        Skip confirmation (for scripts/CI)
    thot debug share --lines 500  Include more log lines
    thot debug share --expire 30  Keep paste for 30 days
    thot debug share --local      Print report locally (no upload)
    thot debug share --no-redact  Disable upload-time secret redaction
    thot debug share --nous       Upload to Nous-internal storage (private)
    thot debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'thot debug share'",
    )

"""
Backup and import commands for thot CLI.

`thot backup` creates a zip archive of the entire ~/.thot/ directory
(excluding the thot-agent repo and transient files).

`thot import` restores from a backup zip, overlaying onto the current
THOT_HOME root.
"""
...
from thot_constants import get_default_thot_root, get_thot_home, display_thot_home
...
# ``thot-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/thot-agent/``
...
# THOT_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "thot-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under THOT_HOME) —
...
# File names that ``thot import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE THOT_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside THOT_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to thot root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``thot-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/thot-agent/`` — must be preserved.
        if part == "thot-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside THOT_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Thot home directory."""
    thot_root = get_default_thot_root()

    if not thot_root.is_dir():
        print(f"Error: Thot home directory not found at {thot_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"thot-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"thot-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_thot_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(thot_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(thot_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``thot-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "thot-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(thot_root)
    ...
    # outside THOT_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_thot_home()}."
        )
    ...
    print(f"\nRestore with: thot import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Thot backup.
    ...
    # Look for telltale files that a thot home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Thot backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.thot/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a thot dir name
        if prefix in {".thot", "thot"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_thot_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (thot_root / "config.yaml").exists()
        has_env = (thot_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Thot configuration.")
        ...
        thot_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under THOT_HOME.
            ...
            target = thot_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(thot_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_thot_home()}."
            )
        ...
        profiles_dir = thot_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # thot_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: thot profile list  (after installing thot)")

        # Guidance
        print()
        if not (thot_root / "thot-agent").is_dir():
            print("Note: The thot-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: thot update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  thot -p {pname} gateway install")

        print("Done. Your Thot configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and thot backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to THOT_HOME).
...
# — `thot update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $THOT_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside THOT_HOME and the entry is silently
...
def _quick_snapshot_root(thot_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(thot_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside THOT_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``thot update``.
    ...
    Args:
        ...
        thot_home: Override for the Thot home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(thot_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for thot backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_thot_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, thot_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``thot_root`` to ``out_path``.
...
def _pre_update_backup_dir(thot_home: Optional[Path] = None) -> Path:
    home = thot_home or get_thot_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of THOT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<THOT_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``thot update``) should continue even if the backup fails.
    """
    thot_root = thot_home or get_default_thot_root()
    if not thot_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(thot_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, thot_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `thot claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of THOT_HOME under ``backups/`` before a
    ``thot claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``thot import <archive>``.  Writes to
    ``<THOT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `thot import` and the

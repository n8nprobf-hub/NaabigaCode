"""
Thot Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.thot/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Thot PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing thot-agent or thot PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Thot Agent" comment and following line
                if '# Thot Agent' in line or '# thot-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('thot' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing thot
                if 'thot' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the thot wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "thot",
        Path("/usr/local/bin/thot"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains thot_cli reference)
                content = wrapper.read_text()
                if 'thot_cli' in content or 'thot-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(thot_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``thot`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Thot home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (thot_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``thot gateway run`` processes
    ...
    try:
        from thot_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from thot_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``THOT_HOME`` and ``THOT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\thot\git\cmd``, ``%LOCALAPPDATA%\thot\git\bin``,
#      ``%LOCALAPPDATA%\thot\git\usr\bin``, ``%LOCALAPPDATA%\thot\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\thot\git\`` and Node to
#      ``%LOCALAPPDATA%\thot\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``thot dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\thot\gateway-service\`` and sometimes
...
def _thot_path_markers(thot_home: Path) -> list[str]:
    """Path-entry substrings that identify Thot-owned User-PATH entries."""
    root = str(thot_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare thot-agent install dir.
    markers = [root + "\\thot-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if THOT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "thot".  We don't want to catch
    # unrelated entries like "cthot-foo" or "ephermeral", so we look for
    # backslash-thot as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(thot_home: Path) -> list[str]:
    """Strip Thot-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _thot_path_markers(thot_home)
            kept: list[str] = []
...
def remove_thot_env_vars_windows() -> list[str]:
    """Delete THOT_HOME and THOT_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("THOT_HOME", "THOT_GIT_BASH_PATH"):
                try:
...
def _is_default_thot_home(thot_home: Path) -> bool:
    """Return True when ``thot_home`` points at the default (non-profile) root."""
    try:
        from thot_constants import get_default_thot_root
        return thot_home.resolve() == get_default_thot_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its THOT_HOME directory.

    We shell out to ``thot -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current THOT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m thot_cli.main` so we don't depend on a `thot`
    #    wrapper that may be half-removed mid-uninstall.
    thot_invocation = [_sys.executable, "-m", "thot_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                thot_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's THOT_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``thot uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$THOT_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from thot_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(thot_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Thot Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Thot Chat GUI installation was found.")
        print(f"  Checked: {thot_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Thot agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(thot_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Thot agent at {thot_home / 'thot-agent'}")
        print(f"  • Your config, sessions, and secrets under {thot_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Thot agent is still installed. Run 'thot' to use the CLI,")
    print("or 'thot uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.thot/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.thot/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie THOT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_thot_home(thot_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.thot data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Thot Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {thot_home / 'config.yaml'}")
    print(f"  Secrets: {thot_home / '.env'}")
    print(f"  Data:    {thot_home / 'cron/'}, {thot_home / 'sessions/'}, {thot_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their THOT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Thot data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Thot code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, thot_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Thot PATH entries from shell configs / Windows User PATH")
    print("  • Thot wrapper scripts and Thot-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Thot config/data: {thot_home}")
        if _is_default_thot_home(thot_home):
        ...
        print(f"  • Keep Thot config/data: {thot_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``thot`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$THOT_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in thot_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\thot\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(thot_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Thot-owned PATH entries in User environment")

        log_info("Removing THOT_HOME / THOT_GIT_BASH_PATH User env vars...")
        removed_env = remove_thot_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Thot-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing thot command...")
    ...
    #     (only when they still point into this Thot home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Thot-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(thot_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Thot-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(thot_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE THOT_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from thot_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(thot_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.thot/, just remove the thot-agent subdir
            if thot_home in project_root.parents or project_root.parent == thot_home:
    ...
    #     under THOT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(thot_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(thot_home)
    ...
    # 5. Optionally remove ~/.thot/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile THOT_HOME dirs live under
        ...
        try:
            if thot_home.exists():
                shutil.rmtree(thot_home)
                log_success(f"Removed {thot_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {thot_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {thot_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  See the repo README: https://github.com/n8nprobf-hub/Thot#readme", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Thot Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m thot_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import thot_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``thot_constants`` + ``thot_cli.colors``
    (and lazily ``thot_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m thot_cli.uninstall")
    parser.add_argument(
        "--mode",

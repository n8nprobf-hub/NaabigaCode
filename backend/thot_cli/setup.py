"""
Interactive setup wizard for Thot Agent.
...
from thot_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://github.com/n8nprobf-hub/Thot#readme"
...
# display_thot_home imported lazily at call sites (stale-module safety during thot update)
...
from thot_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from thot_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Thot Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Thot using environment variables or config commands:")
    print_info("  thot config set model.provider custom")
    print_info("  thot config set model.base_url http://localhost:8080/v1")
    print_info("  thot config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'thot setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``THOT_NONINTERACTIVE=1`` (see ``thot_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("THOT_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``THOT_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from thot_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'thot setup')")


def _print_setup_summary(config: dict, thot_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'thot setup' to configure"))
    ...
    # Video generation — opt-in via `thot tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from thot_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'thot setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'thot setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'thot setup terminal'"))
    ...
    # Spotify (OAuth via thot auth spotify — check auth.json, not env vars)
    try:
        from thot_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'thot setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {thot_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('thot setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('thot setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('thot setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('thot setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('thot setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('thot config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('thot config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('thot config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('thot', Colors.GREEN)}              Start chatting")
    print(f"   {color('thot gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('thot doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `thot tools` and `thot setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``thot model``)
    ...
    provider added to ``thot model`` is automatically available here.
    ...
    # Delegate to the shared thot model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from thot_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: thot model")
    ...
    # on demand via `thot auth add`, `thot setup` vision, and
    # `thot setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``thot model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from thot_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.thot/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Thot auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via thot setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'thot setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Thot runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `thot setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `thot setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into THOT_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("THOT_MAX_ITERATIONS")
    ...
    print_info("  Run `thot setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # THOT_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.thot/logs/tool_calls.log (gateway only)")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Thot delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Thot to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from thot_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/n8nprobf-hub/Thot#readme")
    print()
    ...
    print_info("   https://github.com/n8nprobf-hub/Thot#readme")
    print()
...
def setup_gateway(config: dict):
    ...
    from thot_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Thot from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'thot setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     thot config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_thot_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: thot gateway install --system")
                print_info("  Or run in foreground:  thot gateway")
        else:
            from thot_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   thot gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   thot gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `thot setup tools` and `thot tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``thot_cli.auth`` — lists every supported
    ...
    """
    try:
        from thot_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from thot_cli.config
    ...
    if section_key == "model":
        ...
        from thot_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_thot migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_thot", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Thot.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Thot to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Thot at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Thot at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Thot at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Thot at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Thot equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Thot config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Thot.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Thot's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(thot_home: Path) -> bool:
    ...
    print_info("Thot can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: thot claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: thot claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Thot configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Thot config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Thot (use thot claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      thot setup           — full or quick (auto-detected)
      thot setup model     — just model/provider
      thot setup tts       — just text-to-speech
      thot setup terminal  — just terminal backend
      thot setup gateway   — just messaging platforms
      thot setup tools     — just tool configuration
      thot setup agent     — just agent settings
    """
    from thot_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Thot Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Thot Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Thot Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, thot_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Thot configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'thot setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(thot_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Thot?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, thot_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, thot_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {thot_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'thot config edit'")
    ...
    # Tune later with `thot setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, thot_home)


def _run_first_time_quick_setup(config: dict, thot_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``thot setup <section>``
    or switch providers with ``thot model``.
    """
    ...
    print()
    try:
        from thot_cli.main import _model_flow_nous
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'thot setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    thot setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  thot setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``thot tools`` (which
    ...
    try:
        ...
        from thot_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``thot setup agent`` /
    ``thot config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, thot_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `thot update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        thot tools")
        print_info("  Seed skills:         thot skills opt-in --sync")
        print_info("  Add MCP servers:     thot mcp add")
        print_info("  Enable plugins:      thot plugins")
        print_info("  Tune agent settings: thot setup agent")
        print()
        _print_setup_summary(config, thot_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, thot_home)


def _blank_slate_walkthrough(config: dict, thot_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`thot update` runs from re-injecting them. Opt back in any")
            print_info("time with `thot skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `thot tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `thot plugins list` / `thot plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `thot plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `thot mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `thot mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   thot tools")
    print_info("  Seed skills:         thot skills opt-in --sync")
    print_info("  Add MCP servers:     thot mcp add")
    print_info("  Tune agent settings: thot setup agent")
    print()
...
def _run_quick_setup(config: dict, thot_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'thot setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Thot to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'thot setup gateway'.")

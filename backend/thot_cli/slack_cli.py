"""``thot slack ...`` CLI subcommands.

Today only ``thot slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ thot slack manifest > slack-manifest.json
    # or:
    $ thot slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Thot. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Thot deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from thot_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Thot in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Thot agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``thot_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$THOT_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Thot")
    ...
    """
    name = getattr(args, "name", None) or "Thot"
    description = getattr(args, "description", None) or "Your Thot agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("THOT_HOME") or str(Path.home() / ".thot")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Thot app\n"

"""thot webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    thot webhook subscribe <name> [options]
    thot webhook list
    thot webhook remove <name>
    thot webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.thot/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _thot_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: thot gateway run
"""
...
def webhook_command(args):
    """Entry point for 'thot webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: thot webhook {subscribe|list|remove|test}")
        print("Run 'thot webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (thot gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: thot webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from thot webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (thot gateway run)")

"""CLI handlers for the ``thot proxy`` subcommand."""
...
from thot_cli.proxy.adapters import ADAPTERS, get_adapter
from thot_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "thot proxy requires aiohttp. Install one of:\n"
        "  pip install 'thot-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"thot auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Thot proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Thot proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: thot proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``thot proxy <subcommand>``."""
    ...
    print(
        "thot proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  thot proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  thot proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  thot proxy providers\n"

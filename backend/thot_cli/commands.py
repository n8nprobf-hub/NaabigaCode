"""Slash command definitions and autocomplete for the Thot CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "Lister ou restaurer des points de restauration du système de fichiers", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Créer ou restaurer des instantanés d'état de la config/état de Thot", "Session",
    ...
    CommandDef("steer", "Injecter un message après le prochain appel d'outil sans interrompre", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Définir un objectif permanent sur lequel Thot travaille jusqu'à réalisation", "Session",
    ...
    CommandDef("busy", "Contrôler ce que fait Entrée pendant que Thot travaille", "Configuration",
    ...
    CommandDef("reload-mcp", "Recharger les serveurs MCP depuis la configuration", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scanner ~/.thot/skills/ pour les compétences récemment installées ou supprimées",
    ...
    CommandDef("update", "Mettre à jour Thot Agent vers la dernière version", "Info"),
    CommandDef("version", "Afficher la version de Thot Agent", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`thot_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/thot`` subcommand mapping, and
    ...
    """
    try:
        from thot_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. Thot ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Thot
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``thot skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``thot skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``thot skills list`` and
...
# unrelated command landed. These claim their slots right after /thot,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /thot <command>).
...
# native slot, the alias spelling stays reachable via /thot reset).
...
# ``/thot <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/thot" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /thot credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /thot billing.
#   - moa: high-cost slash mode, available through /thot moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /thot debug on Slack.
_SLACK_VIA_THOT_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/thot <verb>`` subcommand.
    ...
    can still reach them via ``/thot <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/thot`` is always reserved as the first entry so the
    legacy ``/thot <subcommand>`` form keeps working for anything that
    ...
    # Reserve /thot as the catch-all top-level command.
    entries.append(("thot", "Talk to Thot or run a subcommand", "[subcommand] [args]"))
    seen.add("thot")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_THOT_ONLY:
            # Intentionally Slack-via-/thot only (see _SLACK_VIA_THOT_ONLY).
            return
    ...
    # everything except /thot, so a new canonical command can never silently
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /thot handler.

    Maps both canonical names and aliases so /thot bg do stuff works
    the same as /thot background do stuff.

    Plugin-registered slash commands are included so ``/thot <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from thot_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _tools_completions(sub_text: str, sub_lower: str):
        ...
        try:
            ...
            from thot_cli.tools_config import (

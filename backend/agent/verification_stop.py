...
def _session_is_messaging_surface() -> bool:
    ...
    ``THOT_SESSION_PLATFORM``; the CLI and TUI set ``THOT_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``THOT_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("THOT_PLATFORM", "") or os.environ.get(
            "THOT_SESSION_PLATFORM", ""
        )
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``THOT_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("THOT_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `thot-verify-` filename prefix, run it "

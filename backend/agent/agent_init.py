...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: naabiga config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$NAABIGA_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_naabiga_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$NAABIGA_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .naabiga.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / NAABIGA_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.naabiga/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from naabiga_cli.config import load_config as _load_pc_cfg
    ...
    agent._rate_limit_state: Optional["RateLimitState"] = None  # noqa: F821 — forward ref (string annotation)
    ...
    # report cumulative micros spent.  Surfaced behind NAABIGA_DEV_CREDITS.
    ...
    # both live under ~/.naabiga/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from naabiga_logging import setup_logging, setup_verbose_logging
    setup_logging(naabiga_home=_ra()._naabiga_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that naabiga_logging.setup_logging() does not
        ...
        # handler level inside naabiga_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from naabiga_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from naabiga_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `naabiga model` to "
                        "select a provider, or run `naabiga setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff NAABIGA_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.naabiga/sessions/ alongside gateway sessions
    naabiga_home = get_naabiga_home()
    agent.logs_dir = naabiga_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.naabiga/sessions/session_{sid}.json)
    ...
    try:
        from naabiga_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from naabiga_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    _init_kwargs = {
                        ...
                        "naabiga_home": str(get_naabiga_home()),
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "naabiga"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Naabiga' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "naabiga", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, naabiga, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from naabiga_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from naabiga_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Naabiga Agent.  Choose a model with at least "
    ...
    # Nous Naabiga 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from naabiga_cli.model_switch import _check_naabiga_model_warning

            _naabiga_warn = _check_naabiga_model_warning(agent.model or "")
            if _naabiga_warn:
                _user_msg = (
                    "⚠ Nous Research Naabiga 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_naabiga_warn)
        except Exception:
            pass

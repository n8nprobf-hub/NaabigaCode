...
from thot_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: thot config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$THOT_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_thot_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$THOT_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .thot.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / THOT_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.thot/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from thot_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind THOT_DEV_CREDITS.
    ...
    # both live under ~/.thot/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from thot_logging import setup_logging, setup_verbose_logging
    setup_logging(thot_home=_ra()._thot_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that thot_logging.setup_logging() does not
        ...
        # handler level inside thot_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from thot_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from thot_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `thot model` to "
                        "select a provider, or run `thot setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff THOT_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.thot/sessions/ alongside gateway sessions
    thot_home = get_thot_home()
    agent.logs_dir = thot_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.thot/sessions/session_{sid}.json)
    ...
    try:
        from thot_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from thot_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "thot"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Thot' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "thot", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, thot, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from thot_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from thot_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Thot Agent.  Choose a model with at least "
    ...
    # Nous Thot 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from thot_cli.model_switch import _check_thot_model_warning

            _thot_warn = _check_thot_model_warning(agent.model or "")
            if _thot_warn:
                _user_msg = (
                    "⚠ Nous Research Thot 3 & 4 models are NOT agentic — they "

...
``auth.json``.  They intentionally have no dependency on ``thot_cli.auth`` so
...
# Sources Thot owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Thot-owned OAuth/device-code state)

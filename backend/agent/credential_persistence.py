...
``auth.json``.  They intentionally have no dependency on ``naabiga_cli.auth`` so
...
# Sources Naabiga owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Naabiga-owned OAuth/device-code state)

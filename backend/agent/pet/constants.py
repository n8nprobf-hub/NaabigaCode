...
row taxonomy is inferred from the atlas shape so Naabiga can render both legacy
...
# rows (1536x1872), while older Naabiga/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Naabiga' activity state names. They are not always identical to the
...
# Legacy Naabiga/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Naabiga activity names -> accepted row-name aliases in descending

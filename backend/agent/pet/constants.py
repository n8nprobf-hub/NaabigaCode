...
row taxonomy is inferred from the atlas shape so Thot can render both legacy
...
# rows (1536x1872), while older Thot/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Thot' activity state names. They are not always identical to the
...
# Legacy Thot/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Thot activity names -> accepted row-name aliases in descending

...
upstream prefix cache warm.  See ``naabiga-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Naabiga never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NAABIGA_HOME persona while keeping
    ...
    # Pointer to the naabiga-agent skill + docs for user questions about Naabiga itself.
    stable_parts.append(NAABIGA_AGENT_HELP_GUIDANCE)
    ...
    # NAABIGA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Naabiga, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Naabiga profile the agent is running
    # under so it doesn't conflate ~/.naabiga/skills/ (default profile) with
    # ~/.naabiga/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Naabiga profile: default. Other profiles (if any) live "
            "under ~/.naabiga/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Naabiga profile: {active_profile}. This session reads "
            f"and writes ~/.naabiga/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.naabiga/skills/, ~/.naabiga/plugins/, "
            f"~/.naabiga/cron/, ~/.naabiga/memories/ — those belong to a "
    ...
    from naabiga_time import now as _naabiga_now
    now = _naabiga_now()
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Naabiga never rebuilds or reinjects parts of it

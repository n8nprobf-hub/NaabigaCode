...
upstream prefix cache warm.  See ``thot-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Thot never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want THOT_HOME persona while keeping
    ...
    # Pointer to the thot-agent skill + docs for user questions about Thot itself.
    stable_parts.append(THOT_AGENT_HELP_GUIDANCE)
    ...
    # THOT_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Thot, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Thot profile the agent is running
    # under so it doesn't conflate ~/.thot/skills/ (default profile) with
    # ~/.thot/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Thot profile: default. Other profiles (if any) live "
            "under ~/.thot/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Thot profile: {active_profile}. This session reads "
            f"and writes ~/.thot/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.thot/skills/, ~/.thot/plugins/, "
            f"~/.thot/cron/, ~/.thot/memories/ — those belong to a "
    ...
    from thot_time import now as _thot_now
    now = _thot_now()
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Thot never rebuilds or reinjects parts of it

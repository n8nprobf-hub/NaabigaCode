...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `naabiga backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - naabiga_home (str): The active NAABIGA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.naabiga``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "naabiga").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'naabiga memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], naabiga_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'naabiga memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``naabiga_home`` is the active NAABIGA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE NAABIGA_HOME.

        ``naabiga backup`` only walks NAABIGA_HOME, so any provider state kept
        ...
        of the archive, and ``naabiga import`` restores them to their original

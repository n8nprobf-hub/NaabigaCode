...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `thot backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - thot_home (str): The active THOT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.thot``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "thot").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'thot memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], thot_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'thot memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``thot_home`` is the active THOT_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE THOT_HOME.

        ``thot backup`` only walks THOT_HOME, so any provider state kept
        ...
        of the archive, and ``thot import`` restores them to their original

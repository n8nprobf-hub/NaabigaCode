...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Thot with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.thot/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Thot
expects.
"""

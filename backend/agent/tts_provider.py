...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Naabiga with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.naabiga/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Naabiga
expects.
"""

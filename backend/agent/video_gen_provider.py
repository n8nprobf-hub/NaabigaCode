...
as ``kind: backend``) or ``~/.thot/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``thot tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$THOT_HOME/cache/videos/``, creating parents as needed."""
    from thot_constants import get_thot_home

    path = get_thot_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$THOT_HOME/cache/videos/``.

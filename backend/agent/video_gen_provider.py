...
as ``kind: backend``) or ``~/.naabiga/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``naabiga tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$NAABIGA_HOME/cache/videos/``, creating parents as needed."""
    from naabiga_constants import get_naabiga_home

    path = get_naabiga_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$NAABIGA_HOME/cache/videos/``.

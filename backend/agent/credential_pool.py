...
import thot_cli.auth as auth_mod
...
def _iter_custom_providers(config: Optional[dict] = None):
    ...
    if not isinstance(custom_providers, list):
        ...
        try:
            from thot_cli.config import get_compatible_custom_providers
...
def _write_through_provider_state_to_global_root(
    ...
    ``thot_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``thot model`` / ``thot auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Thot process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``thot`` invocation that defaults to the active provider
        (e.g. setup wizard, ``thot auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # thot_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across Thot processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "THOT_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("thot_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Thot
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("thot_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Thot process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Thot profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `thot model` / `thot auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `thot model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``thot auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `thot auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `thot auth remove <provider> <N>` is stable across all source types.
    try:
        from thot_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Thot PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `thot setup` ("Claude
        ...
        # Claude Code CLI, thot_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.thot/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `thot setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"thot_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_thot_oauth_credentials

        for source_name, creds in (
            ("thot_pkce", read_thot_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `thot auth add nous --label <name>`).  Fall back to the
        ...
        # Thot auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from thot_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.thot/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `thot auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `thot auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Thot auth store.  Without this gate the removal is instantly
        ...
        # Thot owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``thot model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``thot auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from thot_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.thot/.env over os.environ — the user's config file is the
    # authoritative source for Thot credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_thot_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `thot auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.thot/.env.
    ...
    try:
        from thot_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from thot_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.thot/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `thot auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Thot

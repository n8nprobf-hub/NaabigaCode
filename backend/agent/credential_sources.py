"""Unified removal contract for every credential source Thot reads from.

Thot seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.thot/.env
    claude_code   — ~/.claude/.credentials.json
    thot_pkce   — ~/.thot/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``thot auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), thot_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.thot/.env  → clear it
    ...
    """
    from thot_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Thot.",
            f"  The pool entry is now suppressed — Thot will ignore "
            f"{env_var} until you run `thot auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Thot stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `thot auth add anthropic` to re-enable if needed.",
    ])


def _remove_thot_pkce(provider: str, removed) -> RemovalResult:
    """~/.thot/.anthropic_oauth.json is ours — delete it outright."""
    from thot_constants import get_thot_home

    result = RemovalResult()
    oauth_file = get_thot_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Thot Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `thot auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `thot auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``thot auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `thot model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Thot auth store is not enough — _seed_from_singletons() would
    ...
    via ``thot auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from thot_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `thot auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `thot auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Thot stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `thot auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="thot_pkce",
        remove_fn=_remove_thot_pkce,
        description="~/.thot/.anthropic_oauth.json",
    ))
    register(RemovalStep(

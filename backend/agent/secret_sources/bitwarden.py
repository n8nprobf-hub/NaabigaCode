...
Naabiga pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.naabiga/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<naabiga_home>/bin/bws`` on
  first use.  Naabiga pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.naabiga/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``naabiga`` invocations don't
  hammer the API.
* Failures NEVER block Naabiga startup.  Missing binary, no network,
...
# In-process cache so repeated load_naabiga_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `naabiga chat -q ...`
...
# <naabiga_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.naabiga/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under naabiga_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$NAABIGA_HOME` / `~/.naabiga` when home is None.
    """
...
def _naabiga_bin_dir() -> Path:
    """Where Naabiga stores its managed binaries.  Profile-aware."""
    from naabiga_constants import get_naabiga_home

    return get_naabiga_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<naabiga_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _naabiga_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``naabiga secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _naabiga_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="naabiga-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "naabiga-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<naabiga_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from naabiga_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_naabiga_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `naabiga secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `naabiga secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `naabiga secrets bitwarden setup` to install."
        )
        return result

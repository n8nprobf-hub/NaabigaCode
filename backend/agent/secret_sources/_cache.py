...
cache problem must never block Thot startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Thot home used for cache paths.

    ``home_path`` is whatever ``load_thot_dotenv()`` already resolved;
    falling back to ``$THOT_HOME`` / ``~/.thot`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("THOT_HOME", Path.home() / ".thot"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<thot_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if THOT_HOME is ever made traversable.
            try:

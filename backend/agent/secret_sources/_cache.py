...
cache problem must never block Naabiga startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Naabiga home used for cache paths.

    ``home_path`` is whatever ``load_naabiga_dotenv()`` already resolved;
    falling back to ``$NAABIGA_HOME`` / ``~/.naabiga`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("NAABIGA_HOME", Path.home() / ".naabiga"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<naabiga_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if NAABIGA_HOME is ever made traversable.
            try:

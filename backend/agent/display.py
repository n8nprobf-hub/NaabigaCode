...
def _diff_ansi() -> dict[str, str]:
    ...
    try:
        from naabiga_cli.skin_engine import get_active_skin
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Naabiga' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("NAABIGA_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
# Tools classified as read-only (never mutate state) — shown as a green
# ``[read-only]`` badge like Claude Code. Anything write-capable or with
# side effects is ``[destructive]`` (red); everything else falls back to
# no badge (Claude Code shows ``[open-world]`` only for non-MCP tools
# with unknown side effects; we keep the default implicit).
_READ_ONLY_TOOLS = frozenset({
    "read_file", "search_files", "web_search", "web_extract", "browser_snapshot",
    "browser_console", "browser_get_images", "session_search", "skills_list",
    "skill_view", "todo", "process",
})
_DESTRUCTIVE_TOOLS = frozenset({
    "write_file", "patch", "terminal", "execute_code", "browser_navigate",
    "browser_click", "browser_type", "browser_press", "browser_scroll",
    "browser_back", "memory", "skill_manage", "cronjob", "send_message",
    "image_generate", "text_to_speech", "vision_analyze", "delegate_task",
    "clarify",
})


def get_tool_classification(tool_name: str) -> str:
    """Return ``read-only``, ``destructive`` or ``open-world`` for a tool.

    Mirrors Claude Code's semantic tool badges: ``[read-only]`` in the
    success color for tools that never mutate state, ``[destructive]`` in
    the error color for write-capable / side-effect tools. Everything else
    is ``open-world`` (unknown side effects).
    """
...
def get_claude_tool_call_header(tool_name: str, args: dict | None) -> str:
    """Build the ``Name(args)`` header of a Claude Code-style tool call.

    ``Name`` in bold, ``(args)`` dimmed — the primary argument is the
    tool's natural preview (command for terminal, path for file tools,
    query for search, …), falling back to a compact JSON summary.
    """
    args = args or {}
    args = redact_tool_args_for_display(tool_name, args) or args
    ...
    if not preview:
        try:
            ...
            preview = ""
    inner = preview if preview else ""
    return f"{_BOLD}{tool_name}{_RESET}({_DIM}{inner}{_RESET})"


def get_tool_state_marker(is_error: bool) -> str:
    """Return the Claude Code completion marker: ``✔`` success, ``✖`` error."""
    return f"{_RED}✖{_RESET}" if is_error else f"{_GREEN}✔{_RESET}"


def get_claude_tool_line(
    tool_name: str,
    args: dict | None,
    duration: float = 0.0,
    ...
    """Render a Claude Code-style tool call line.

    ``┊ Name(args) [read-only] ⠿ Running`` while running, then
    ``┊ Name(args) [read-only] ✔ 1.2s`` (success) or ``✖ 1.2s`` (error).
    """
    header = get_claude_tool_call_header(tool_name, args)
    classification = get_tool_classification(tool_name)
    if classification == "read-only":
        badge = f" {_GREEN}[read-only]{_RESET}"
    elif classification == "destructive":
        badge = f" {_RED}[destructive]{_RESET}"
    else:
        badge = f" {_DIM}[open-world]{_RESET}"

    if state == "running":
        return f"┊ {header}{badge} {_DIM}⠿ Running{_RESET}"

    marker = get_tool_state_marker(is_error)
    dur = f"{duration:.1f}s" if duration else ""
    tail = f" {dur}" if dur else ""
    line = f"┊ {header}{badge} {marker}{tail}"
    if is_error:
        _, failure_suffix = _detect_tool_failure(tool_name, result)
        if failure_suffix:
            line += f"{_RED}{failure_suffix}{_RESET}"
    return line

...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Naabiga-Session-Id header), and an
        ...
        if env_var_enabled("NAABIGA_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from naabiga_cli.config import load_config as _load_moa_cfg
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to naabiga's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from naabiga_cli.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from naabiga_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if function_name == "todo":
        ...
        def _execute(next_args: dict) -> Any:
            ...
            if not session_db:
                from naabiga_state import format_session_db_unavailable
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: naabiga' own dispatch loop intentionally keeps an

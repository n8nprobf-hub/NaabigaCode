...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.naabiga/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Naabiga ↔ models.dev
# ---------------------------------------------------------------------------

# Naabiga provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Naabiga (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_naabiga_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``naabiga config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Naabiga provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from naabiga_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Naabiga agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Naabiga provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Naabiga ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Naabiga or models.dev provider ID.  Tries exact match then

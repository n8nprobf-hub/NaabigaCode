...
Owns one Codex thread per Thot session. Drives `turn/start`, consumes
...
# Thot' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_THOT_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Thot/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Thot session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _THOT_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("THOT_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="thot",
            client_title="Thot Agent",
            client_version=_get_thot_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Thot' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Thot' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own thot-tools callback we
            # auto-accept — the user already approved Thot' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Thot approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Thot returns 'once', 'session', 'always', or 'deny'.
...
def _get_thot_version() -> str:
    """Best-effort Thot version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("thot-agent")

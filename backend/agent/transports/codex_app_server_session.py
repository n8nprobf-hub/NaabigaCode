...
Owns one Codex thread per Naabiga session. Drives `turn/start`, consumes
...
# Naabiga' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_NAABIGA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Naabiga/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Naabiga session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _NAABIGA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("NAABIGA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="naabiga",
            client_title="Naabiga Agent",
            client_version=_get_naabiga_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Naabiga' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Naabiga' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own naabiga-tools callback we
            # auto-accept — the user already approved Naabiga' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Naabiga approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Naabiga returns 'once', 'session', 'always', or 'deny'.
...
def _get_naabiga_version() -> str:
    """Best-effort Naabiga version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("naabiga-agent")

"""Upload a Naabiga session transcript to Hugging Face as an agent trace.

Naabiga stores sessions in its own SQLite store (``naabiga_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``naabiga trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Naabiga' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "naabiga-traces"
_NAABIGA_VERSION = "naabiga-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Naabiga OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Naabiga' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Naabiga conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.naabiga/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `naabiga trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/naabiga-traces`` dataset. Returns a

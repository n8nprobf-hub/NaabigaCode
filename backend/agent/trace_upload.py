"""Upload a Thot session transcript to Hugging Face as an agent trace.

Thot stores sessions in its own SQLite store (``thot_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``thot trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Thot' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "thot-traces"
_THOT_VERSION = "thot-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Thot OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Thot' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Thot conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.thot/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `thot trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/thot-traces`` dataset. Returns a

...
Plugins built on Naabiga Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Naabiga
...
:class:`~naabiga_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Naabiga' main config
...
handles every provider, fallback chain, and per-task override Naabiga
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`naabiga_cli.plugins.PluginContext`

...
Plugins built on Thot Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Thot
...
:class:`~thot_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Thot' main config
...
handles every provider, fallback chain, and per-task override Thot
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`thot_cli.plugins.PluginContext`

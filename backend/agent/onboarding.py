...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Thot starts and finds ``~/.openclaw/``.

    Points users at ``thot claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``thot claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Thot, run "
        "`thot claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `thot claw cleanup` (renames it to ~/.openclaw.pre-migration — "

...
# Matches ${THOT_SKILL_DIR} / ${THOT_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${THOT_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(THOT_SKILL_DIR|THOT_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${THOT_SKILL_DIR} / ${THOT_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "THOT_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "THOT_SESSION_ID" and session_id:

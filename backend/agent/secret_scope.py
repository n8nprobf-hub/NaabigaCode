...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the NAABIGA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Naabiga runtime / deployment
    "NAABIGA_HOME", "NAABIGA_PROFILE", "NAABIGA_GATEWAY_LOCK_DIR",
    "NAABIGA_MAX_ITERATIONS", "NAABIGA_MAX_TOKENS", "NAABIGA_API_TIMEOUT",
    "NAABIGA_REDACT_SECRETS", "NAABIGA_NOUS_TIMEOUT_SECONDS",
    "_NAABIGA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "NAABIGA_KANBAN_DB", "NAABIGA_KANBAN_WORKSPACES_ROOT", "NAABIGA_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "NAABIGA_KANBAN_",
    "NAABIGA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(naabiga_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(naabiga_home) / ".env")

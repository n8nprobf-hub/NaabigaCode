...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the THOT_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Thot runtime / deployment
    "THOT_HOME", "THOT_PROFILE", "THOT_GATEWAY_LOCK_DIR",
    "THOT_MAX_ITERATIONS", "THOT_MAX_TOKENS", "THOT_API_TIMEOUT",
    "THOT_REDACT_SECRETS", "THOT_NOUS_TIMEOUT_SECONDS",
    "_THOT_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "THOT_KANBAN_DB", "THOT_KANBAN_WORKSPACES_ROOT", "THOT_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "THOT_KANBAN_",
    "THOT_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(thot_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(thot_home) / ".env")

"""Mixins d'AIAgent — extraits de run_agent.py.

Généré par extraction AST. AIAgent (run_agent.py) hérite de ces mixins;
chaque mixin contient un groupe thématique de méthodes. Les helpers
déplacés (_is_ephemeral_scaffolding, _safe_session_filename_component,
_qwen_portal_headers, _routermint_headers) vivent ici pour éviter tout
import circulaire.
"""
import logging
...
# Module-level logger + markers (appelés par nom nu depuis les méthodes des
# mixins — comme dans run_agent.py d'origine).
logger = logging.getLogger(__name__)
_DB_PERSISTED_MARKER = "_db_persisted"


# Helpers partagés (top-level — appelés par nom nu depuis les mixins)
            ...
            *,
            ...
            if normalized_provider == "copilot":
                try:
                    ...
                    pass
            return _AgentStreamMixin._model_requires_responses_api(model)
...
class _AgentPersistMixin:
    ...
    def _coerce_api_error_detail(value: Any) -> str:
            ...
            if isinstance(value, dict):
                ...
                for key in ("message", "detail", "error", "code", "type"):
                    if key in value:
                        nested_detail =  _AgentPersistMixin._coerce_api_error_detail(value[key])
                        if nested_detail:
            ...
            if isinstance(value, (list, tuple)):
                parts = [
                     _AgentPersistMixin._coerce_api_error_detail(item)
                    for item in value
                ]
    ...
    def _summarize_api_error(error: Exception) -> str:
            ...
            if isinstance(body, dict):
                ...
                if msg:
                    ...
                    msg =  _AgentPersistMixin._coerce_api_error_detail(msg)
                    return _AgentPersistMixin._decorate_xai_entitlement_error(f"{prefix}{msg[:300]}")
            ...
            status_code = getattr(error, "status_code", None)
            prefix = f"HTTP {status_code}: " if status_code else ""
            return _AgentPersistMixin._decorate_xai_entitlement_error(f"{prefix}{raw[:500]}")
...
class _AgentApiMixin:
    ...
    def _materialize_data_url_for_vision(image_url: str) -> tuple[str, Optional[Path]]:
            ...
            if len(data) > _AgentApiMixin._MAX_DATA_URL_BASE64_BYTES:
                logger.warning(

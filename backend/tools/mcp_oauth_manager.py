...
`mcp_tool.py`, and `thot_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ThotMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_thot_provider_class() -> Optional[type]:
    ...
    class ThotMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._thot_server_name = server_name
            ...
            self._thot_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ThotTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ThotTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._thot_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ThotTokenStorage
                        if isinstance(storage, ThotTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._thot_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ThotTokenStorage
            if not isinstance(storage, ThotTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``thot mcp reauth``.
            ...
            back to ``thot mcp reauth``.
            """
            try:
                if self._thot_preregistered:
                    return
    ...
    return ThotMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_THOT_PROVIDER_CLS: Optional[type] = _make_thot_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ThotMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `thot mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``thot mcp remove <name>`` and (indirectly) by
        ``thot mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:

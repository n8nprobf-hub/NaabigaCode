...
provider** — turn on a backend by enabling a plugin (``naabiga plugins
enable video_gen/<name>``) and selecting it in ``naabiga tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/naabiga-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `naabiga tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `naabiga tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `naabiga tools` → "
...
# `naabiga tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`naabiga tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `naabiga tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}

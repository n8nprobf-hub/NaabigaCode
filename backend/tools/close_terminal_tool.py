#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Thot desktop GUI.
...
``read_terminal`` it is gated on ``THOT_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — THOT_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("THOT_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Thot desktop GUI (the tabs mirroring terminal(background=true) runs). "

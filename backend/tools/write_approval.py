...
(see ``naabiga_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<NAABIGA_HOME>/pending/{memory,skills}/<id>.json``
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    if subsystem not in _SUBSYSTEMS:
        return False
    try:
        from naabiga_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _pending_dir(subsystem: str) -> Path:
    return get_naabiga_home() / "pending" / subsystem

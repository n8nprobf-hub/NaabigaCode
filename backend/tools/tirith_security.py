...
it is automatically downloaded from GitHub releases to $THOT_HOME/bin/tirith.
...
def _get_thot_home() -> str:
    """Return the Thot home directory, respecting THOT_HOME env var."""
    return str(get_thot_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_thot_home(), ".tirith-install-failed")
...
def _thot_bin_dir() -> str:
    """Return $THOT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_thot_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $THOT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_thot_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $THOT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $THOT_HOME/bin/tirith
    ...
    thot_bin = os.path.join(_thot_bin_dir(), "tirith")
    if os.path.isfile(thot_bin) and os.access(thot_bin, os.X_OK):
        _resolved_path = thot_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return thot_bin

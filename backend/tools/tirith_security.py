...
it is automatically downloaded from GitHub releases to $NAABIGA_HOME/bin/tirith.
...
def _get_naabiga_home() -> str:
    """Return the Naabiga home directory, respecting NAABIGA_HOME env var."""
    return str(get_naabiga_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_naabiga_home(), ".tirith-install-failed")
...
def _naabiga_bin_dir() -> str:
    """Return $NAABIGA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_naabiga_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NAABIGA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_naabiga_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NAABIGA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NAABIGA_HOME/bin/tirith
    ...
    naabiga_bin = os.path.join(_naabiga_bin_dir(), "tirith")
    if os.path.isfile(naabiga_bin) and os.access(naabiga_bin, os.X_OK):
        _resolved_path = naabiga_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return naabiga_bin

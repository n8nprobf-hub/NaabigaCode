...
def auth_json_path():
    """Return the Thot auth store path, respecting THOT_HOME overrides."""
    return get_thot_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`thot tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from thot_cli.auth import resolve_nous_access_token

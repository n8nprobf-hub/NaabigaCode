...
_FORCE_SYNC_ENV = "NAABIGA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.naabiga") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.naabiga to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``NAABIGA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, naabiga_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.naabiga/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="naabiga-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.naabiga/skills/a.md`` →
        ``~/.naabiga/skills/a.md``, a new remote file at
        ``/root/.naabiga/skills/b.md`` maps to ``~/.naabiga/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []

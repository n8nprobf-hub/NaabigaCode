...
# Must match naabiga_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``naabiga-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. naabiga-cli) so that individual
    ...
    try:
        ...
        if delegation_effort or delegation_effort is False:
            from naabiga_constants import parse_reasoning_effort
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.naabiga/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_naabiga_dir("cache/delegation", "delegation_cache")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'naabiga auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Naabiga config.

    Prefer the shared persistent loader because it follows the active
    NAABIGA_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``NAABIGA_IGNORE_USER_CONFIG=1`` (``naabiga chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("NAABIGA_IGNORE_USER_CONFIG") == "1"
    if not prefer_legacy:
        try:
            from naabiga_cli.config import load_config_readonly

...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <NAABIGA_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  NAABIGA_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_naabiga_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: NAABIGA_HOME env var, then ~/.naabiga
            home = os.environ.get("NAABIGA_HOME") or os.path.expanduser("~/.naabiga")

...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <THOT_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  THOT_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_thot_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: THOT_HOME env var, then ~/.thot
            home = os.environ.get("THOT_HOME") or os.path.expanduser("~/.thot")

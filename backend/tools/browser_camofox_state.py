"""Naabiga-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Naabiga
...
def get_camofox_state_dir() -> Path:
    ...
    return get_naabiga_home() / CAMOFOX_STATE_DIR_NAME / CAMOFOX_STATE_SUBDIR


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Naabiga-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Naabiga profile = same userId).
    ...
    return {
        "user_id": f"naabiga_{user_digest}",

...
<url>``) is spawned as a direct child of the Thot process. Thot's own
...
Thot process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Thot startup's opt-in

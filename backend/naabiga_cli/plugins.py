"""
Naabiga Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with naabiga-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.naabiga/plugins/<name>/``
3. **Project plugins** – ``./.naabiga/plugins/<name>/`` (opt-in via
   ``NAABIGA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``naabiga_agent.plugins``
   entry-point group.
...
from naabiga_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``NAABIGA_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("NAABIGA_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``NAABIGA_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.naabiga/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("NAABIGA_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NAABIGA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Naabiga loggers.
    """
    ...
    logger.debug(
        "NAABIGA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Naabiga' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by naabiga_cli.kanban_db when a task
    ...
    # separate `naabiga -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `naabiga kanban dispatch`),
...
ENTRY_POINTS_GROUP = "naabiga_agent.plugins"

_NS_PARENT = "naabiga_plugins"
...
class PluginManifest:
    ...
    #              in ~/.naabiga/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``naabiga plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Naabiga profile name (e.g. ``"default"``).

        Derived from ``NAABIGA_HOME`` via
        :func:`naabiga_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.naabiga/profiles/<name>``, or ``"custom"`` when
        ``NAABIGA_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Naabiga core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``naabiga honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``naabiga <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        """Naabiga: dashboard auth providers were removed with the dashboard."""
        logger.info(
            "Naabiga is CLI-only: dashboard-auth provider registration is a no-op "
            "(plugin '%s' tried to register %r).",
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_naabiga_dotenv()`` startup — after
        ``~/.naabiga/.env`` loads, before Naabiga reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Naabiga' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``naabiga model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: naabiga_cli.main imports plugins indirectly
        from naabiga_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.naabiga/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("NAABIGA_SAFE_MODE"):
            logger.info("NAABIGA_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to naabiga_cli/. Two layouts are
        ...
        # 2. User plugins (~/.naabiga/plugins/)
        user_dir = get_naabiga_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.naabiga/plugins/)
        if _env_enabled("NAABIGA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".naabiga" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set NAABIGA_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with naabiga and must
            ...
            # all ~20 of them added several seconds to every `naabiga`
            # invocation — including plain `naabiga chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Naabiga ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `naabiga plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``naabiga plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`naabiga plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `naabiga plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``naabiga_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``naabiga_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="naabiga-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``naabiga tools`` TUI so plugin-provided toolsets appear

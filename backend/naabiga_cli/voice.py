...
# lookup, and thot-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and thot-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when THOT_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("THOT_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def speak_text(text: str) -> None:
    ...
    try:
        ...
        os.makedirs(os.path.join(tempfile.gettempdir(), "thot_voice"), exist_ok=True)

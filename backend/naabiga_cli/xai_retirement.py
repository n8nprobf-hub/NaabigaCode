...
Pure logic: walks a Thot config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `thot doctor` and a future `thot migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Thot config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Thot config and return retirement issues.
...
def apply_migration(
    ...
    from thot_cli.config import require_readable_config_before_write

...
``$THOT_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-thot-setup
(the stage2 hook) has chowned the volume and seeded $THOT_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$THOT_HOME``,
    not under ``profiles/``). The dispatcher in ``thot_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``thot gateway start`` (no ``-p``) targets. Without it,
    bare ``thot gateway start`` inside the container would land on
    ...
    ``$THOT_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        thot_home: The container's THOT_HOME (typically /opt/data).
            Profiles live under ``<thot_home>/profiles/<name>/``;
            the default profile lives at ``<thot_home>`` itself.
    ...
    # ``thot gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(thot_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(thot_home)
    ...
    profiles_root = thot_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `thot profile create` (config.yaml
            # is not — that comes later via `thot setup`). Use it as the
            ...
            # via ``thot -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(thot_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = thot_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("THOT_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the thot args.
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    ``thot`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``thot <subcommand>``, so an
    explicit leading ``thot`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `thot <subcommand>`; peel an explicit thot.
    if args and Path(args[0]).name == "thot":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``thot dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted THOT_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `thot -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit thot
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the thot user) won't hit EACCES. See
...
def _write_reconcile_log(
    thot_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $THOT_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted THOT_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        thot_home=thot_home, scandir=scandir,
    )
    for a in actions:
        print(

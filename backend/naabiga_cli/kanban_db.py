...
``<root>`` is the **shared Naabiga root** (the parent of any active
...
``naabiga -p <profile>`` joins the same board as the dispatcher that
...
the profile's Naabiga home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``NAABIGA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``NAABIGA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``naabiga kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.naabiga``. In Docker / custom
deployments where ``NAABIGA_HOME`` points outside ``~/.naabiga`` (e.g.
``/opt/hermes``), ``<root>`` is ``NAABIGA_HOME``. Legacy env-var
overrides still work:

* ``NAABIGA_KANBAN_DB`` — pin the database file path directly.
* ``NAABIGA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``NAABIGA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``NAABIGA_KANBAN_DB``,
``NAABIGA_KANBAN_WORKSPACES_ROOT``, and ``NAABIGA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/naabiga-kanban-v1-spec.pdf`` for the full
...
from naabiga_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active NAABIGA_HOME so dispatcher- and
...
# ``NAABIGA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``NAABIGA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("NAABIGA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``NAABIGA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("NAABIGA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``NAABIGA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "NAABIGA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "naabiga_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``naabiga-agent``
...
def kanban_home() -> Path:
    """Return the shared Naabiga root that anchors the kanban board.

    Resolution order:

    1. ``NAABIGA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_naabiga_root()``, which already returns ``<root>``
       when ``NAABIGA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NAABIGA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NAABIGA_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("NAABIGA_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from naabiga_constants import get_default_naabiga_root
    return get_default_naabiga_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``naabiga kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``NAABIGA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``naabiga kanban boards
    ...
    env = os.environ.get("NAABIGA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``naabiga kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``NAABIGA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("NAABIGA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``NAABIGA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("NAABIGA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``NAABIGA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("NAABIGA_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``naabiga kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``NAABIGA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (naabiga_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``NAABIGA_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("NAABIGA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``naabiga gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Naabiga treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``NAABIGA_KANBAN_DB`` env → ``NAABIGA_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `naabiga kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from naabiga_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See naabiga_state._WAL_INCOMPAT_MARKERS for detection logic.
                from naabiga_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``naabiga kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``NAABIGA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from naabiga_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``naabiga --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from naabiga_cli import projects_db as _pdb
    ...
    # `naabiga --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``naabiga kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `naabiga kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``naabiga kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``naabiga kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``naabiga kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``naabiga kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``NAABIGA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Naabiga' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Naabiga-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``naabiga kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Naabiga checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Naabiga treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Naabiga creates/reuses that linked worktree. With
      no ``workspace_path``, Naabiga anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Naabiga uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``NAABIGA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Naabiga
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``naabiga kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``naabiga kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Naabiga profile.
    ...
    if not rows:
        return False
    try:
        from naabiga_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from naabiga_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Naabiga profile.
        # `_default_spawn` invokes ``naabiga -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Naabiga
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `naabiga kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from naabiga_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_naabiga_argv() -> list[str]:
    """Return the interpreter-bound Naabiga CLI invocation."""
    # ``naabiga_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``naabiga`` package — there is no
    # ``naabiga`` package to import.
    return [sys.executable, "-m", "naabiga_cli.main"]


def _absolute_naabiga_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Naabiga shim."""
...
def _naabiga_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Naabiga executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_naabiga_argv()
    return [_absolute_naabiga_path(path)]


def _resolve_naabiga_argv() -> list[str]:
    """Resolve the ``naabiga`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$NAABIGA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("naabiga")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\naabiga.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m naabiga_cli.main`` — fallback for setups where
       Naabiga is launched from a venv and the ``naabiga`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_naabiga_bin`` for the same reason. Kept
    local (not imported from gateway) because ``naabiga_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("NAABIGA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _naabiga_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _naabiga_path_argv(resolved_env_bin)
        return _module_naabiga_argv()

    naabiga_bin = _safe_which_no_cwd("naabiga") if _IS_WINDOWS else shutil.which("naabiga")
    if naabiga_bin:
        return _naabiga_path_argv(naabiga_bin)
    return _module_naabiga_argv()
...
def _resolve_worker_cli_toolsets(naabiga_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``NAABIGA_KANBAN_TASK`` is set.
    """
    if not naabiga_home:
        return None
    try:
        from naabiga_constants import reset_naabiga_home_override, set_naabiga_home_override
        from naabiga_cli.config import load_config
        from naabiga_cli.tools_config import _get_platform_tools

        token = set_naabiga_home_override(naabiga_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for NAABIGA_HOME=%r (%s)",
            naabiga_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``naabiga -p <profile> chat -q ...`` subprocess.
    ...
    ``NAABIGA_KANBAN_DB`` / ``NAABIGA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject NAABIGA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `naabiga -p <name>` the
    # _apply_profile_override() runs *before* naabiga_constants is imported.
    # If NAABIGA_HOME is absent from the child's env, get_naabiga_home() falls
    # back to Path.home() / ".naabiga" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from naabiga_cli.profiles import resolve_profile_env
    try:
        env["NAABIGA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via NAABIGA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NAABIGA_HOME never had profiles created.
        pass
    if task.tenant:
        env["NAABIGA_TENANT"] = task.tenant
    env["NAABIGA_KANBAN_TASK"] = task.id
    env["NAABIGA_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["NAABIGA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["NAABIGA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["NAABIGA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["NAABIGA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["NAABIGA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`naabiga -p <name>`
    # rewrites NAABIGA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_naabiga_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["NAABIGA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["NAABIGA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["NAABIGA_KANBAN_BOARD"] = resolved_board
    # NAABIGA_PROFILE is the author the kanban_comment tool defaults to.
    # `naabiga -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["NAABIGA_PROFILE"] = profile_arg

    cmd = [
        *_resolve_naabiga_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped NAABIGA_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("NAABIGA_HOME"))
    if worker_toolsets:
    ...
    # `naabiga kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`naabiga` executable not found on PATH. "
            "Install Naabiga Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``naabiga kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled NAABIGA_PROFILE values like "naabiga-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Naabiga root exists

    Reads profile paths directly so this module has no import dependency on
    ``naabiga_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from naabiga_constants import get_default_naabiga_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``naabiga kanban assignees`` for the terminal.

"""
naabiga fallback — manage the fallback provider chain.
...
Subcommands:
  naabiga fallback [list]   Show the current fallback chain (default when no subcommand)
  naabiga fallback add      Pick provider + model via the same picker as `naabiga model`,
                           then append the selection to the chain
  naabiga fallback remove   Pick an entry to delete from the chain
  naabiga fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.naabiga/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from naabiga_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `naabiga model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  naabiga fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/n8nprobf-hub/Naabiga#readme")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `naabiga model`, then append the selection to the chain."""
    from naabiga_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `naabiga model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `naabiga fallback list` to view, or `naabiga fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``naabiga fallback [subcommand]``."""

"""
thot fallback — manage the fallback provider chain.
...
Subcommands:
  thot fallback [list]   Show the current fallback chain (default when no subcommand)
  thot fallback add      Pick provider + model via the same picker as `thot model`,
                           then append the selection to the chain
  thot fallback remove   Pick an entry to delete from the chain
  thot fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.thot/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from thot_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `thot model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  thot fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/n8nprobf-hub/Thot#readme")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `thot model`, then append the selection to the chain."""
    from thot_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `thot model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `thot fallback list` to view, or `thot fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``thot fallback [subcommand]``."""

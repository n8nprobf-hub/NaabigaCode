"""``thot logs`` — view and filter Thot log files.
...
Usage examples::
    ...
    thot logs                    # last 50 lines of agent.log
    thot logs -f                 # follow agent.log in real time
    thot logs errors             # last 50 lines of errors.log
    thot logs gateway -n 100    # last 100 lines of gateway.log
    thot logs gui -f            # follow gui.log (dashboard/pty/ws)
    thot logs desktop -f        # follow desktop.log (Electron app boot/backend)
    thot logs --level WARNING    # only WARNING+ lines
    thot logs --session abc123   # filter by session ID substring
    thot logs --component tools  # only tool-related lines
    thot logs --since 1h         # lines from the last hour
    thot logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_thot_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Thot runs — try 'thot chat' first)")
        sys.exit(1)
    ...
    if component:
        from thot_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_thot_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_thot_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_thot_home()}/logs/")
        return

    print(f"Log files in {display_thot_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'thot chat' to generate logs)")

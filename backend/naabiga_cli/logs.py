"""``naabiga logs`` — view and filter Naabiga log files.
...
Usage examples::
    ...
    naabiga logs                    # last 50 lines of agent.log
    naabiga logs -f                 # follow agent.log in real time
    naabiga logs errors             # last 50 lines of errors.log
    naabiga logs gateway -n 100    # last 100 lines of gateway.log
    naabiga logs gui -f            # follow gui.log (dashboard/pty/ws)
    naabiga logs desktop -f        # follow desktop.log (Electron app boot/backend)
    naabiga logs --level WARNING    # only WARNING+ lines
    naabiga logs --session abc123   # filter by session ID substring
    naabiga logs --component tools  # only tool-related lines
    naabiga logs --since 1h         # lines from the last hour
    naabiga logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_naabiga_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Naabiga runs — try 'naabiga chat' first)")
        sys.exit(1)
    ...
    if component:
        from naabiga_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_naabiga_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_naabiga_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_naabiga_home()}/logs/")
        return

    print(f"Log files in {display_naabiga_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'naabiga chat' to generate logs)")

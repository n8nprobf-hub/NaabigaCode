"""Helpers for loading Naabiga .env files consistently across entrypoints."""
...
# load_naabiga_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_naabiga_dotenv().
# Used by setup / `naabiga model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# NAABIGA_HOME paths we've already pulled external secrets for during this
# process.  ``load_naabiga_dotenv()`` is called at module-import time from
# several hot modules (cli.py, naabiga_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_naabiga_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which NAABIGA_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `naabiga setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``naabiga_cli.config._sanitize_env_lines`` which
    already knows all valid Naabiga env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from naabiga_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_naabiga_dotenv(
    *,
    naabiga_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Naabiga environment files with user config taking precedence.

    Behavior:
    - `~/.naabiga/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(naabiga_home or os.getenv("NAABIGA_HOME", Path.home() / ".naabiga"))
    ...
    #   EnvironmentFile=-/path/to/.naabiga/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of NAABIGA_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Naabiga reads
    ...
    owns the once-per-NAABIGA_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_naabiga_dotenv()`` runs at import
    time from several hot modules (cli.py, naabiga_cli/main.py,
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `naabiga model`

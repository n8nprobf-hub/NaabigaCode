"""
Configuration management for Thot Agent.

Config files are stored in ~/.thot/ for easy access:
- ~/.thot/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.thot/.env         - API keys and secrets

This module provides:
- thot config          - Show current configuration
- thot config edit     - Open config in editor
- thot config set      - Set a specific value
- thot config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``thot config set`` (which rewrites
    ...
    ``config.yaml`` in place: thot never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.thot/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``thot logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  thot config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Thot makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Thot itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``thot update``, the TUI build.
...
#   on every plugin install / ``thot update``.
...
# * ``THOT_HOME`` / ``THOT_PROFILE`` / ``THOT_CONFIG`` /
#   ``THOT_ENV`` — Thot runtime location flags. Writing these into
...
#
# IMPORTANT: ``THOT_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (THOT_LANGFUSE_PUBLIC_KEY,
# THOT_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Thot runtime location — never via dashboard env writer.
    # NOT a THOT_* blanket: integration credentials (THOT_GEMINI_*,
    # THOT_LANGFUSE_*, THOT_SPOTIFY_*, ...) ARE allowed.
    "THOT_HOME", "THOT_PROFILE", "THOT_CONFIG", "THOT_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Thot runtime location "
            "(THOT_HOME, THOT_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "THOT_TOOL_PROGRESS", "THOT_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `thot plugins enable
    # observability/langfuse` or `thot tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "THOT_LANGFUSE_ENV",
    "THOT_LANGFUSE_RELEASE",
    "THOT_LANGFUSE_SAMPLE_RATE",
    "THOT_LANGFUSE_MAX_CHARS",
    "THOT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from thot_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("THOT_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_thot_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Thot is running in package-manager-managed mode.

    Two signals: the THOT_MANAGED env var (set by the systemd service),
    or a .managed marker file in THOT_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade thot-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``thot_cli/`` — i.e. the git checkout for source
    ...
    interpreter, NOT of ``$THOT_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Thot was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$THOT_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. THOT_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.thot``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$THOT_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.thot:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$THOT_HOME``, a
    ...
    ``docker`` and ``thot update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/thot-agent`` image bakes a ``docker``
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $THOT_HOME.
    try:
    ...
    #    host install whose shared $THOT_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``thot update``.
    try:
        method = (
            (get_thot_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``thot_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$THOT_HOME``, so that two installs sharing one data directory
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Thot lives in a ``uv tool`` layout.

    ``uv tool install thot-agent`` places the install at
    ``.../uv/tools/thot-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``thot-agent`` happens to be uv-tool-installed on the machine while
    the *active* Thot is a regular pip/venv install, causing
    ``thot update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/thot-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade thot-agent"
    if method == "docker":
        return "docker pull nousresearch/thot-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade thot-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade thot-agent"
        return "pip install --upgrade thot-agent"
    return "thot update"
...
# Long-form text for ``thot update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# thot_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Thot, it doesn't update
...
✗ ``thot update`` doesn't apply inside the Docker container.

Thot Agent runs as a published image (nousresearch/thot-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/thot-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate thot-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/thot-agent:latest --version
...
Notes:
  ...
  • Your config and session history live under ``$THOT_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``thot update`` inside Docker.
...
def format_managed_message(action: str = "modify this Thot installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("THOT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Thot installation is managed by NixOS "
            f"(THOT_MANAGED={env_hint}).\n"
            "Edit services.thot-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Thot installation is managed by Homebrew "
            f"(THOT_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade thot-agent"
        )

    return (
        f"Cannot {action}: this Thot installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Thot."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from THOT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, thot_bin
    or None if container mode is not active, we're already inside the
    container, or THOT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("THOT_DEV") == "1":
        return None
    ...
    container_mode_file = get_thot_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "thot-agent")
    exec_user = info.get("exec_user", "thot")
    thot_bin = info.get("thot_bin", "/data/current-package/bin/thot")
...
# Re-export from thot_constants — canonical definition lives there.
from thot_constants import get_thot_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_thot_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_thot_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_thot_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the THOT_UID / THOT_GID env vars set by Docker deployments.

    Docker containers running Thot commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level THOT_HOME once, but
    subdirectories created at runtime by ``ensure_thot_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("THOT_UID", "").strip()
    gid_str = os.environ.get("THOT_GID", "").strip()
    try:
...
def _chown_to_thot_uid(path) -> None:
    """Chown ``path`` to ``THOT_UID:THOT_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_thot_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_thot_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the thot group can
    share state with the gateway service.

    The mode can be overridden via the THOT_HOME_MODE environment variable
    (e.g. THOT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse THOT_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``THOT_UID``/``THOT_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("THOT_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_thot_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Thot runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("THOT_CONTAINER") or os.environ.get("THOT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set THOT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into THOT_HOME, upgrading legacy empty templates.
...
def ensure_thot_home():
    """Ensure ~/.thot directory structure exists with secure permissions.
    ...
    """
    home = get_thot_home()
    # Named profiles must be created explicitly (e.g. ``thot profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old THOT_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_thot_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_thot_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"THOT_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Thot-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Thot
        ...
        # identity slot (SOUL.md). Empty by default. The THOT_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Thot adds a coding operating brief
    ...
    "terminal": {
        ...
        #             THOT_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force THOT_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Thot auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Thot sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Thot
        ...
        #  "/home/user/.thot/cache/documents:/output"]
        ...
        # bundled Thot image, which drops to the `thot` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Thot sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Thot should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``thot chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.thot/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: thot sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .thot.md, or .cursorrules before Thot applies
    ...
    # single read_file page exceeds these limits, Thot truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Thot' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); thot =
                                      # Thot' compression threshold triggers
    ...
    # See tools/kanban_tools.py and thot_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``thot dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``thot kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``thot kanban decompose`` and the kanban
        ...
        # ``thot profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `thot model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `thot` (and `thot chat`) launches by default:
        ...
        # REPL and `--tui` (or THOT_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `thot --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `thot -c` muscle memory.  Default off so existing
        # users aren't surprised.  THOT_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `thot --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to THOT_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/thot`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `thot pets`. Disabled until
        ...
        "pet": {
            ...
            # get_thot_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``THOT_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``THOT_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``THOT_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``THOT_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``THOT_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Thot appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.thot/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.thot/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.thot/cache/delegation/
    ...
    # response. If not, Thot feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Thot auto-pauses the goal and
    ...
    "moa": {
        ...
        # <thot_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.thot/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${THOT_SKILL_DIR} and ${THOT_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `thot curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `thot curator run --consolidate` overrides
        ...
        # normally restored on every `thot update`, so pruning them only
        ...
        # skipped), snapshot ~/.thot/skills/ into
        # ~/.thot/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `thot curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for thot-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Thot Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (THOT_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Thot' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.thot/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or THOT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `thot doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``thot_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Thot to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $THOT_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via THOT_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `thot -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `thot` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `thot kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     thot-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Thot tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.thot/logs/.
    ...
    # update model picker lists without shipping a thot-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `thot model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal THOT_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.thot/sessions/sessions.json entirely.
        ...
        # the THOT_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.thot.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.thot/.env,
        ...
        # behavior: files must live under the Thot cache, under
        ...
        # user. Bridged to THOT_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Thot
        # cache (~/.thot/cache/{documents,images,audio,video,screenshots})
        ...
        # to THOT_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # THOT_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to THOT_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.thot/state.db.
    ...
    "sessions": {
        ...
        # default of ``thot sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.thot/sessions/session_{sid}.json`` on every turn
    ...
    # ``thot update`` behaviour.
    "updates": {
        # Run a full ``thot backup``-style zip of THOT_HOME before every
        # ``thot update``.  Backups land in ``<THOT_HOME>/backups/`` and
        # can be restored with ``thot import <path>``.  Off by default:
        # zipping a large THOT_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``thot update
        ...
        "backup_keep": 5,
        # What `thot update` does with uncommitted local changes to the
        ...
        # Refresh an already-installed cua-driver during `thot update`.
    ...
    "lsp": {
        ...
        #              ``<THOT_HOME>/lsp/bin/`` on first use.
    ...
    # `thot tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.thot/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.thot/.env (or your shell) and never in config.yaml.
            ...
            # ~/.thot/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.thot/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Thot disables it for our users unless they
        # explicitly opt in here. When false (default), Thot sets
    ...
    # Thot Desktop (Electron app) launch options. These only affect
    # `thot desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the THOT_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Thot mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'thot model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Thot auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Thot, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @thot:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. THOT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #thot)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: thot-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'thot-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Thot API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Thot API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Thot API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # THOT_TOOL_PROGRESS and THOT_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.thot.config`` entries, then checks
    ...
    try:
        ...
        # should never break `thot update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Thot' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Thot versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``thot_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'thot setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Thot won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'thot doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("THOT_HOME", "~/.thot")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "thot update / thot -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("THOT_TOOL_PROGRESS")
            old_mode = get_env_value("THOT_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from THOT_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from THOT_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("THOT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from THOT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$THOT_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_thot_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `thot plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `thot curator status` has no
    ...
    #   3. Creates `~/.thot/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_thot_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_thot_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `thot config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `thot config set`"
                    )
    ...
    # behavior set it to true explicitly via `thot config set`.
    ...
    try:
        ...
        from thot_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: thot config set <key> <value>")
    ...
    # `thot update`, but only the version bump is persisted.
    ...
    # metadata.thot.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_thot_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``thot config set``
    ...
    backends) — while display paths (``thot status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _thot_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.thot/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.thot/config.yaml.
    ...
    (which change ``THOT_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/thot/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_thot_dotenv()
...
#   openai-codex (OAuth — thot auth) — OpenAI Codex
#   nous         (OAuth — thot auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.thot/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Thot writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.thot/.env.
    ...
    interactive menu render (`thot tools`, `thot setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `thot tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Thot env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.thot/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.thot/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.thot/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.thot/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Thot — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Thot vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.thot/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.thot/.env`` over ``os.environ``.

    Used for Thot-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Thot Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from thot_cli.auth import get_anthropic_key
    ...
    # Warn on stale THOT_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("THOT_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale THOT_MAX_ITERATIONS={_env_ghost} "
                f"(run 'thot doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  thot config edit     # Edit config file", Colors.DIM))
    print(color("  thot config set <key> <value>", Colors.DIM))
    print(color("  thot setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `thot config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `thot config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: thot config set <key> <value>")
            print()
            print("Examples:")
            print("  thot config set model anthropic/claude-sonnet-4")
            print("  thot config set terminal.backend docker")
            print("  thot config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'thot config migrate' to add them")
        ...
        print("  thot config           Show current configuration")
        print("  thot config edit      Open config in editor")
        print("  thot config set <key> <value>   Set a config value")
        print("  thot config check     Check for missing/outdated config")
        print("  thot config migrate   Update config with new options")
        print("  thot config path      Show config file path")
        print("  thot config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``thot config`` UI so users

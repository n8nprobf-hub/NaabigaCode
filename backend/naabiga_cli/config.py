"""
Configuration management for Naabiga Agent.

Config files are stored in ~/.naabiga/ for easy access:
- ~/.naabiga/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.naabiga/.env         - API keys and secrets

This module provides:
- naabiga config          - Show current configuration
- naabiga config edit     - Open config in editor
- naabiga config set      - Set a specific value
- naabiga config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``naabiga config set`` (which rewrites
    ...
    ``config.yaml`` in place: naabiga never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.naabiga/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``naabiga logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  naabiga config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Naabiga makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Naabiga itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``naabiga update``, the TUI build.
...
#   on every plugin install / ``naabiga update``.
...
# * ``NAABIGA_HOME`` / ``NAABIGA_PROFILE`` / ``NAABIGA_CONFIG`` /
#   ``NAABIGA_ENV`` — Naabiga runtime location flags. Writing these into
...
#
# IMPORTANT: ``NAABIGA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NAABIGA_LANGFUSE_PUBLIC_KEY,
# NAABIGA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Naabiga runtime location — never via dashboard env writer.
    # NOT a NAABIGA_* blanket: integration credentials (NAABIGA_GEMINI_*,
    # NAABIGA_LANGFUSE_*, NAABIGA_SPOTIFY_*, ...) ARE allowed.
    "NAABIGA_HOME", "NAABIGA_PROFILE", "NAABIGA_CONFIG", "NAABIGA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Naabiga runtime location "
            "(NAABIGA_HOME, NAABIGA_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.naabiga/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "NAABIGA_TOOL_PROGRESS", "NAABIGA_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `naabiga plugins enable
    # observability/langfuse` or `naabiga tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NAABIGA_LANGFUSE_ENV",
    "NAABIGA_LANGFUSE_RELEASE",
    "NAABIGA_LANGFUSE_SAMPLE_RATE",
    "NAABIGA_LANGFUSE_MAX_CHARS",
    "NAABIGA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NAABIGA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_naabiga_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Naabiga is running in package-manager-managed mode.

    Two signals: the NAABIGA_MANAGED env var (set by the systemd service),
    or a .managed marker file in NAABIGA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade naabiga-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``naabiga_cli/`` — i.e. the git checkout for source
    ...
    interpreter, NOT of ``$NAABIGA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Naabiga was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$NAABIGA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. NAABIGA_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.naabiga``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$NAABIGA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.naabiga:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$NAABIGA_HOME``, a
    ...
    ``docker`` and ``naabiga update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/naabiga-agent`` image bakes a ``docker``
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $NAABIGA_HOME.
    try:
    ...
    #    host install whose shared $NAABIGA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``naabiga update``.
    try:
        method = (
            (get_naabiga_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``naabiga_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$NAABIGA_HOME``, so that two installs sharing one data directory
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Naabiga lives in a ``uv tool`` layout.

    ``uv tool install naabiga-agent`` places the install at
    ``.../uv/tools/naabiga-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``naabiga-agent`` happens to be uv-tool-installed on the machine while
    the *active* Naabiga is a regular pip/venv install, causing
    ``naabiga update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/naabiga-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade naabiga-agent"
    if method == "docker":
        return "docker pull nousresearch/naabiga-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade naabiga-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade naabiga-agent"
        return "pip install --upgrade naabiga-agent"
    return "naabiga update"
...
# Long-form text for ``naabiga update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# naabiga_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Naabiga, it doesn't update
...
✗ ``naabiga update`` doesn't apply inside the Docker container.

Naabiga Agent runs as a published image (nousresearch/naabiga-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/naabiga-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate naabiga-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/naabiga-agent:latest --version
...
Notes:
  ...
  • Your config and session history live under ``$NAABIGA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``naabiga update`` inside Docker.
...
def format_managed_message(action: str = "modify this Naabiga installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NAABIGA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Naabiga installation is managed by NixOS "
            f"(NAABIGA_MANAGED={env_hint}).\n"
            "Edit services.naabiga-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Naabiga installation is managed by Homebrew "
            f"(NAABIGA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade naabiga-agent"
        )

    return (
        f"Cannot {action}: this Naabiga installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Naabiga."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NAABIGA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, naabiga_bin
    or None if container mode is not active, we're already inside the
    container, or NAABIGA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NAABIGA_DEV") == "1":
        return None
    ...
    container_mode_file = get_naabiga_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "naabiga-agent")
    exec_user = info.get("exec_user", "naabiga")
    naabiga_bin = info.get("naabiga_bin", "/data/current-package/bin/naabiga")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "naabiga_bin": naabiga_bin,
    }
...
# Re-export from naabiga_constants — canonical definition lives there.
from naabiga_constants import get_naabiga_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_naabiga_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_naabiga_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_naabiga_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the NAABIGA_UID / NAABIGA_GID env vars set by Docker deployments.

    Docker containers running Naabiga commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NAABIGA_HOME once, but
    subdirectories created at runtime by ``ensure_naabiga_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NAABIGA_UID", "").strip()
    gid_str = os.environ.get("NAABIGA_GID", "").strip()
    try:
...
def _chown_to_naabiga_uid(path) -> None:
    """Chown ``path`` to ``NAABIGA_UID:NAABIGA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_naabiga_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_naabiga_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the naabiga group can
    share state with the gateway service.

    The mode can be overridden via the NAABIGA_HOME_MODE environment variable
    (e.g. NAABIGA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NAABIGA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NAABIGA_UID``/``NAABIGA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NAABIGA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_naabiga_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Naabiga runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NAABIGA_CONTAINER") or os.environ.get("NAABIGA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NAABIGA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NAABIGA_HOME, upgrading legacy empty templates.
...
def ensure_naabiga_home():
    """Ensure ~/.naabiga directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``naabiga profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old NAABIGA_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_naabiga_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_naabiga_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NAABIGA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Naabiga-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Naabiga
        ...
        # identity slot (SOUL.md). Empty by default. The NAABIGA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Naabiga adds a coding operating brief
    ...
    "terminal": {
        ...
        #             NAABIGA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force NAABIGA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Naabiga auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Naabiga sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Naabiga
        ...
        #  "/home/user/.naabiga/cache/documents:/output"]
        ...
        # bundled Naabiga image, which drops to the `naabiga` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Naabiga sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Naabiga should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``naabiga chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.naabiga/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: naabiga sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .naabiga.md, or .cursorrules before Naabiga applies
    ...
    # single read_file page exceeds these limits, Naabiga truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Naabiga' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); naabiga =
                                      # Naabiga' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``naabiga kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``naabiga kanban decompose`` and the kanban
        ...
        # ``naabiga profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `naabiga model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `naabiga` (and `naabiga chat`) launches by default:
        ...
        # REPL and `--tui` (or NAABIGA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `naabiga --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `naabiga -c` muscle memory.  Default off so existing
        # users aren't surprised.  NAABIGA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `naabiga --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to NAABIGA_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/naabiga`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `naabiga pets`. Disabled until
        ...
        "pet": {
            ...
            # get_naabiga_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``NAABIGA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NAABIGA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``NAABIGA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``NAABIGA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``NAABIGA_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Naabiga appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.naabiga/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.naabiga/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.naabiga/cache/delegation/
    ...
    # response. If not, Naabiga feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Naabiga auto-pauses the goal and
    ...
    "moa": {
        ...
        # <naabiga_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.naabiga/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NAABIGA_SKILL_DIR} and ${NAABIGA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `naabiga curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `naabiga curator run --consolidate` overrides
        ...
        # normally restored on every `naabiga update`, so pruning them only
        ...
        # skipped), snapshot ~/.naabiga/skills/ into
        # ~/.naabiga/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `naabiga curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for naabiga-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Naabiga Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NAABIGA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Naabiga' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.naabiga/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NAABIGA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `naabiga doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``naabiga_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Naabiga to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $NAABIGA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via NAABIGA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `naabiga -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `naabiga` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `naabiga kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     naabiga-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Naabiga tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.naabiga/logs/.
    ...
    # update model picker lists without shipping a naabiga-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `naabiga model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal NAABIGA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.naabiga/sessions/sessions.json entirely.
        ...
        # the NAABIGA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.naabiga.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.naabiga/.env,
        ...
        # behavior: files must live under the Naabiga cache, under
        ...
        # user. Bridged to NAABIGA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Naabiga
        # cache (~/.naabiga/cache/{documents,images,audio,video,screenshots})
        ...
        # to NAABIGA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NAABIGA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NAABIGA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.naabiga/state.db.
    ...
    "sessions": {
        ...
        # default of ``naabiga sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.naabiga/sessions/session_{sid}.json`` on every turn
    ...
    # ``naabiga update`` behaviour.
    "updates": {
        # Run a full ``naabiga backup``-style zip of NAABIGA_HOME before every
        # ``naabiga update``.  Backups land in ``<NAABIGA_HOME>/backups/`` and
        # can be restored with ``naabiga import <path>``.  Off by default:
        # zipping a large NAABIGA_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``naabiga update
        ...
        "backup_keep": 5,
        # What `naabiga update` does with uncommitted local changes to the
        ...
        # Refresh an already-installed cua-driver during `naabiga update`.
    ...
    "lsp": {
        ...
        #              ``<NAABIGA_HOME>/lsp/bin/`` on first use.
    ...
    # `naabiga tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.naabiga/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.naabiga/.env (or your shell) and never in config.yaml.
            ...
            # ~/.naabiga/bin/ on first use.  When False you must install
            ...
            # `naabiga secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.naabiga/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Naabiga disables it for our users unless they
        # explicitly opt in here. When false (default), Naabiga sets
    ...
    # Naabiga Desktop (Electron app) launch options. These only affect
    # `naabiga desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the NAABIGA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Naabiga mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'naabiga model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Naabiga auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Naabiga, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @naabiga:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NAABIGA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #naabiga)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: naabiga-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'naabiga-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Naabiga API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Naabiga API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Naabiga API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NAABIGA_TOOL_PROGRESS and NAABIGA_TOOL_PROGRESS_MODE are deprecated —
    ...
    "NAABIGA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.naabiga.config`` entries, then checks
    ...
    try:
        ...
        # should never break `naabiga update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Naabiga' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Naabiga versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``naabiga_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'naabiga setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Naabiga won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'naabiga doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NAABIGA_HOME", "~/.naabiga")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "naabiga update / naabiga -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NAABIGA_TOOL_PROGRESS")
            old_mode = get_env_value("NAABIGA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NAABIGA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NAABIGA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NAABIGA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NAABIGA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NAABIGA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_naabiga_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `naabiga plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `naabiga curator status` has no
    ...
    #   3. Creates `~/.naabiga/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_naabiga_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_naabiga_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `naabiga config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `naabiga config set`"
                    )
    ...
    # behavior set it to true explicitly via `naabiga config set`.
    ...
    try:
        ...
        from naabiga_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: naabiga config set <key> <value>")
    ...
    # `naabiga update`, but only the version bump is persisted.
    ...
    # metadata.naabiga.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_naabiga_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``naabiga config set``
    ...
    backends) — while display paths (``naabiga status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _naabiga_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.naabiga/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.naabiga/config.yaml.
    ...
    (which change ``NAABIGA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/naabiga/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_naabiga_dotenv()
...
#   openai-codex (OAuth — naabiga auth) — OpenAI Codex
#   nous         (OAuth — naabiga auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.naabiga/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Naabiga writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.naabiga/.env.
    ...
    interactive menu render (`naabiga tools`, `naabiga setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `naabiga tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Naabiga env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.naabiga/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.naabiga/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.naabiga/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.naabiga/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Naabiga — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Naabiga vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.naabiga/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.naabiga/.env`` over ``os.environ``.

    Used for Naabiga-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Naabiga Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from naabiga_cli.auth import get_anthropic_key
    ...
    # Warn on stale NAABIGA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("NAABIGA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale NAABIGA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'naabiga doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  naabiga config edit     # Edit config file", Colors.DIM))
    print(color("  naabiga config set <key> <value>", Colors.DIM))
    print(color("  naabiga setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `naabiga config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `naabiga config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: naabiga config set <key> <value>")
            print()
            print("Examples:")
            print("  naabiga config set model anthropic/claude-sonnet-4")
            print("  naabiga config set terminal.backend docker")
            print("  naabiga config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'naabiga config migrate' to add them")
        ...
        print("  naabiga config           Show current configuration")
        print("  naabiga config edit      Open config in editor")
        print("  naabiga config set <key> <value>   Set a config value")
        print("  naabiga config check     Check for missing/outdated config")
        print("  naabiga config migrate   Update config with new options")
        print("  naabiga config path      Show config file path")
        print("  naabiga config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``naabiga config`` UI so users

...
Used by ``naabiga profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``naabiga_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Naabiga Agent kanban board.
...
Rules:
  ...
  - Never write "Naabiga Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from naabiga_constants import get_naabiga_home  # type: ignore
            profile_dir = Path(get_naabiga_home())
        else:

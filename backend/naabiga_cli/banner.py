...
Pure display functions with no ThotCLI state dependency.
"""
...
from thot_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

THOT_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# (e.g. nix-built thot — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/n8nprobf-hub/Thot.git"
_OFFICIAL_REPO_CANONICAL = "github.com/n8nprobf-hub/thot"
...
def _fetch_pypi_latest(package: str = "thot-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Thot update is available.

    Two paths: if ``THOT_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = thot_home / ".update_check"
    embedded_rev = os.environ.get("THOT_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # THOT_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `thot update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/thot/update/check`
    ...
    if embedded_rev:
        ...
        # $THOT_HOME/thot-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = thot_home / "thot-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Thot git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$THOT_HOME/thot-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``thot_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from thot_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Thot checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/thot-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Thot v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else THOT_CADUCEUS
    except Exception:
        _bskin = None
        _hero = THOT_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from thot_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from thot_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built thot; we know an update
    ...
    # Pip-install warning — `pip install thot-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else THOT_AGENT_LOGO
        console.print(_logo)
        console.print()

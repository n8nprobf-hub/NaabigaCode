"""naabiga webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    naabiga webhook subscribe <name> [options]
    naabiga webhook list
    naabiga webhook remove <name>
    naabiga webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.naabiga/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _naabiga_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_naabiga_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     naabiga gateway setup
  ...
  Then start the gateway: naabiga gateway run
"""
...
def webhook_command(args):
    """Entry point for 'naabiga webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: naabiga webhook {subscribe|list|remove|test}")
        print("Run 'naabiga webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (naabiga gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: naabiga webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from naabiga webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (naabiga gateway run)")

"""``thot gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.thot/auth.json``) — the same path ``thot dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.thot/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `thot auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from thot_cli.auth import AuthError, resolve_nous_access_token
    from thot_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `thot gateway enroll` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `thot setup` (or `thot auth add nous`) first, then retry.")
        else:
    ...
    #    save_env_value writes them to ~/.thot/.env (0600 dir) and never logs.
    to_write = {
    ...
    from thot_cli.config import get_env_path

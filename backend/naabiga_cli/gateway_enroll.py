"""``naabiga gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.naabiga/auth.json``) — the same path ``naabiga dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.naabiga/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `naabiga auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from naabiga_cli.auth import AuthError, resolve_nous_access_token
    from naabiga_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `naabiga gateway enroll` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `naabiga setup` (or `naabiga auth add nous`) first, then retry.")
        else:
    ...
    #    save_env_value writes them to ~/.naabiga/.env (0600 dir) and never logs.
    to_write = {
    ...
    from naabiga_cli.config import get_env_path

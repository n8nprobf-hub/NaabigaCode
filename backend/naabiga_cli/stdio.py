...
on Japanese locales, etc.).  Naabiga's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``naabiga_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Naabiga is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NAABIGA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NAABIGA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Naabiga-managed Git install directories so
    ...
    # already-running shells don't see the change — which means naabiga
    ...
    # install without Naabiga-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Naabiga's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Naabiga's
    ``naabiga config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Naabiga (User env var in Windows Settings, or
    export in a PowerShell profile) and Naabiga picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Naabiga-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\naabiga\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``naabiga`` from right after install) retain their old PATH.

    Any subprocess Naabiga spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Naabiga-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "naabiga", "git", "cmd"),
        os.path.join(local_appdata, "naabiga", "git", "bin"),
        os.path.join(local_appdata, "naabiga", "git", "usr", "bin"),
        # Naabiga venv Scripts directory — host of the naabiga.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes naabiga, but harmless to include.
        os.path.join(local_appdata, "naabiga", "naabiga-agent", "venv", "Scripts"),

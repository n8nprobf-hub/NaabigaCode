"""`thot checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.thot/checkpoints/``.  Actions:

    thot checkpoints               # same as `status`
    thot checkpoints status        # total size, project count, breakdown
    thot checkpoints list          # per-project checkpoint counts + workdir
    thot checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    thot checkpoints clear [-f]    # nuke the entire base (asks first)
    thot checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    thot checkpoints
    thot checkpoints prune --retention-days 3 --max-size-mb 200
    thot checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: thot checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``thot checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `thot checkpoints` → status

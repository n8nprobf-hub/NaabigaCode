"""`naabiga checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.naabiga/checkpoints/``.  Actions:

    naabiga checkpoints               # same as `status`
    naabiga checkpoints status        # total size, project count, breakdown
    naabiga checkpoints list          # per-project checkpoint counts + workdir
    naabiga checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    naabiga checkpoints clear [-f]    # nuke the entire base (asks first)
    naabiga checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    naabiga checkpoints
    naabiga checkpoints prune --retention-days 3 --max-size-mb 200
    naabiga checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: naabiga checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``naabiga checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `naabiga checkpoints` → status

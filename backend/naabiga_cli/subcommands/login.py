...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``thot login`` was removed in favor of ``thot auth`` / ``thot model``
    (the runtime handler in ``thot_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``thot login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``thot --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``thot login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``thot model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `thot auth` to manage credentials, "
            "`thot model` to select a provider, or `thot setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `thot model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: thot-cli)"
    )

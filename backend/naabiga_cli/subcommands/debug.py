"""``naabiga debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Naabiga Agent. Use 'naabiga debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    naabiga debug share              Upload debug report (asks for confirmation)
    naabiga debug share --yes        Skip confirmation (for scripts/CI)
    naabiga debug share --lines 500  Include more log lines
    naabiga debug share --expire 30  Keep paste for 30 days
    naabiga debug share --local      Print report locally (no upload)
    naabiga debug share --no-redact  Disable upload-time secret redaction
    naabiga debug share --nous       Upload to Nous-internal storage (private)
    naabiga debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'naabiga debug share'",
    )

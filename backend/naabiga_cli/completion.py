"""Shell completion script generation for thot CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_thot_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Thot Agent bash completion
# Add to ~/.bashrc:
#   eval "$(thot completion bash)"

_thot_profiles() {{
    local profiles_dir="$HOME/.thot/profiles"
...
_thot_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_thot_profiles)" -- "$cur"))
        return
    fi
...
complete -F _thot_completion thot
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef thot
# Thot Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(thot completion zsh)"

_thot_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.thot/profiles" ]]; then
        profiles+=($HOME/.thot/profiles/*(N/:t))
    fi
...
_thot() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_thot_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'thot command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Thot Agent fish completion",
        "# Add to your config:",
        "#   thot completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __thot_profiles",
        "    echo default",
        "    if test -d $HOME/.thot/profiles",
        "        for d in $HOME/.thot/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c thot -f -s p -l profile"
        " -d 'Profile name' -xa '(__thot_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__thot_profiles)' -d 'Profile name'"
                )

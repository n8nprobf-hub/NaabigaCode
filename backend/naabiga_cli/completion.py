"""Shell completion script generation for naabiga CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_naabiga_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Naabiga Agent bash completion
# Add to ~/.bashrc:
#   eval "$(naabiga completion bash)"

_naabiga_profiles() {{
    local profiles_dir="$HOME/.naabiga/profiles"
...
_naabiga_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_naabiga_profiles)" -- "$cur"))
        return
    fi
...
complete -F _naabiga_completion naabiga
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef naabiga
# Naabiga Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(naabiga completion zsh)"

_naabiga_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.naabiga/profiles" ]]; then
        profiles+=($HOME/.naabiga/profiles/*(N/:t))
    fi
...
_naabiga() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_naabiga_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'naabiga command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Naabiga Agent fish completion",
        "# Add to your config:",
        "#   naabiga completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __naabiga_profiles",
        "    echo default",
        "    if test -d $HOME/.naabiga/profiles",
        "        for d in $HOME/.naabiga/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c naabiga -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c naabiga -f -s p -l profile"
        " -d 'Profile name' -xa '(__naabiga_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__naabiga_profiles)' -d 'Profile name'"
                )

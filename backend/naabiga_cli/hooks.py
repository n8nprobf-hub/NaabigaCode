"""naabiga hooks — inspect and manage shell-script hooks.

Usage::

    naabiga hooks list
    naabiga hooks test <event> [--for-tool X] [--payload-file F]
    naabiga hooks revoke <command>
    naabiga hooks doctor

Consent records live under ``~/.naabiga/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.naabiga/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``naabiga hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: naabiga hooks {list|test|revoke|doctor}")
        print("Run 'naabiga hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.naabiga/config.yaml.")
        print("See `naabiga hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `naabiga hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `naabiga hooks test` and `naabiga hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Naabiga wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `naabiga hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `naabiga hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `naabiga hooks doctor`.")

"""Windows ConPTY bridge for the `thot dashboard` chat tab.

Drop-in counterpart to ``thot_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``thot_cli.web_server`` consumes: ``spawn``, ``read``,

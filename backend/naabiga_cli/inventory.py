...
def load_picker_context() -> ConfigContext:
    ...
    """
    from naabiga_cli.config import get_compatible_custom_providers, load_config
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``naabiga model`` CLI picker. Adds network calls
    ...
    try:
        from naabiga_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _apply_capabilities(rows: list[dict]) -> None:
    ...
    """
    from naabiga_cli.models import model_supports_fast_mode
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from naabiga_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Naabiga.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `naabiga model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from naabiga_cli.models import CANONICAL_PROVIDERS
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`naabiga_cli.model_switch.list_picker_providers`) so the

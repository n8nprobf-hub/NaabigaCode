...
the tool registry or launching Thot (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``thot-cli`` to the non-existent ``thot``. ``resolve_toolset('thot')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``thot-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``thot`` instead of ``thot-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `thot tools` to reconfigure."
        )
    return warnings

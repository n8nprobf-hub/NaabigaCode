...
the tool registry or launching Naabiga (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``naabiga-cli`` to the non-existent ``naabiga``. ``resolve_toolset('naabiga')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``naabiga-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``naabiga`` instead of ``naabiga-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `naabiga tools` to reconfigure."
        )
    return warnings

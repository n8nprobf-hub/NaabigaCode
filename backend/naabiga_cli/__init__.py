"""
Naabiga CLI - Unified command-line interface for Naabiga (CLI-only Naabiga fork).

Provides subcommands for:
- naabiga chat          - Interactive chat (same as ./naabiga)
- naabiga gateway       - Run gateway in foreground
- naabiga gateway start - Start gateway service
...
def _ensure_utf8():
    ...
    can even start — e.g. `naabiga setup` on a fresh Pi.
    ...
    naabiga_cli/stdio.py::configure_windows_stdio() runs later from the entry

"""``thot debug`` debug tools for Thot Agent.

Currently supports:
    thot debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.thot/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[thot debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.thot/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``thot debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``thot debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `thot debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "thot-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``thot debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.thot/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``thot debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "thot-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ThotDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from thot_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_thot_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``thot dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``thot dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "thot-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``thot debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "thot debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `thot debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  thot debug delete <url>")

    print("\nShare these links with the Thot team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Thot version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``thot debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from thot_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "thot debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `thot debug share --local` to print the report instead, "
            "or `thot debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: thot debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'thot debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``thot debug`` call.
    ...
    # best-effort — any failure is swallowed so ``thot debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: thot debug <command>")
        print()
        print("Commands:")

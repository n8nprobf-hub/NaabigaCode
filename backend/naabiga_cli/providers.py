"""
Single source of truth for provider identity in Thot Agent.
...
2. **Thot overlays** — transport type, auth patterns, aggregator flags,
...
# -- Thot overlay ----------------------------------------------------------
# Thot-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ThotOverlay:
    """Thot-specific provider metadata layered on top of models.dev."""
...
THOT_OVERLAYS: Dict[str, ThotOverlay] = {
    "moa": ThotOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": ThotOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": ThotOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": ThotOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ThotOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ThotOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="THOT_QWEN_BASE_URL",
    ),
    "lmstudio": ThotOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ThotOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ThotOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ThotOverlay(
    ...
    "minimax-oauth": ThotOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ThotOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ThotOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": ThotOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": ThotOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ThotOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "thot", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Thot overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Thot overlay
    ...
    overlay = THOT_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + thot extra
    ...
    if overlay is not None:
        # Thot-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in THOT_OVERLAYS
    if provider == "bedrock":

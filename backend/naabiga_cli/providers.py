"""
Single source of truth for provider identity in Naabiga Agent.
...
2. **Naabiga overlays** — transport type, auth patterns, aggregator flags,
...
# -- Naabiga overlay ----------------------------------------------------------
# Naabiga-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class NaabigaOverlay:
    """Naabiga-specific provider metadata layered on top of models.dev."""
...
NAABIGA_OVERLAYS: Dict[str, NaabigaOverlay] = {
    "moa": NaabigaOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": NaabigaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": NaabigaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": NaabigaOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": NaabigaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": NaabigaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="NAABIGA_QWEN_BASE_URL",
    ),
    "lmstudio": NaabigaOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": NaabigaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": NaabigaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": NaabigaOverlay(
    ...
    "kimi-for-coding": NaabigaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": NaabigaOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": NaabigaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": NaabigaOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": NaabigaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": NaabigaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": NaabigaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": NaabigaOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": NaabigaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": NaabigaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": NaabigaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": NaabigaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": NaabigaOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": NaabigaOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": NaabigaOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": NaabigaOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": NaabigaOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": NaabigaOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "naabiga", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Naabiga overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Naabiga overlay
    ...
    overlay = NAABIGA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + naabiga extra
    ...
    if overlay is not None:
        # Naabiga-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in NAABIGA_OVERLAYS
    if provider == "bedrock":

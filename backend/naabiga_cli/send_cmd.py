"""CLI subcommand: ``naabiga send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"naabiga send: {file_path} is not a text file. --file reads the "
                ...
                f'  naabiga send --to telegram "MEDIA:{file_path}"\n'
                f'  naabiga send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  naabiga send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"naabiga send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"naabiga send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"naabiga send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"naabiga send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"naabiga send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `naabiga gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.naabiga/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_naabiga_env() -> None:
    """Populate ``os.environ`` from ``~/.naabiga/.env`` AND bridge top-level
    ...
    startup that ``naabiga send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.naabiga/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.naabiga/config.yaml`` into
       ...
       via ``naabiga config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``naabiga send``
    ...
    try:
        from naabiga_cli.config import _expand_env_vars
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.naabiga/.env and ~/.naabiga/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "naabiga send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  naabiga send --to telegram \"hello\"\n"
            "  naabiga send --to discord:#ops --file report.md\n"
            "  naabiga send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "naabiga send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `naabiga send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Naabiga "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.naabiga/.env + ~/.naabiga/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  naabiga send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | naabiga send --to telegram:-1001234567890\n"
            "  naabiga send --to discord:#ops --file /tmp/report.md\n"
            "  naabiga send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  naabiga send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  naabiga send --list                  # all platforms\n"
            "  naabiga send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `naabiga send --list telegram`.",
    )

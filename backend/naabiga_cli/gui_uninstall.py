"""
Naabiga Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``naabiga desktop`` / ``naabiga gui``)
     Built inside the agent checkout under ``$NAABIGA_HOME/naabiga-agent/``:
       ...
       - ``$NAABIGA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Naabiga.app`` or ``~/Applications/Naabiga.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Naabiga`` (NSIS per-user)
...
the app name ("Naabiga"), separate from ``$NAABIGA_HOME``:
  - macOS:   ``~/Library/Application Support/Naabiga``
  - Windows: ``%APPDATA%\\Naabiga``
  - Linux:   ``$XDG_CONFIG_HOME/Naabiga`` (default ``~/.config/Naabiga``)
...
``naabiga uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(naabiga_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return naabiga_home / "naabiga-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Naabiga"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Naabiga"
...
def source_built_gui_artifacts(naabiga_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``naabiga desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``naabiga-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(naabiga_home)
    ...
    return [
        ...
        naabiga_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Naabiga" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Naabiga).
            local_base / "Programs" / "Naabiga",
            # Older / alternate layout some builds used.
            local_base / "naabiga-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Naabiga")
    else:
        ...
        paths += [
            data_base / "applications" / "naabiga.desktop",
            data_base / "applications" / "Naabiga.desktop",
        ]
    return paths


def agent_is_installed(naabiga_home: Path) -> bool:
    """Return True when a usable Python agent install exists under NAABIGA_HOME.
    ...
    if (agent_root / "naabiga_cli").is_dir():
        return True
...
def gui_is_installed(naabiga_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(naabiga_home):
        if p.exists():
            return True
...
def gui_install_summary(naabiga_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = naabiga_home if naabiga_home is not None else get_naabiga_home()
...
def uninstall_gui(naabiga_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``naabiga-agent/naabiga_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$NAABIGA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove naabiga' or "
            "'sudo dnf remove naabiga'). AppImage builds are a single file you "

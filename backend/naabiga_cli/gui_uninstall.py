"""
Thot Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``thot desktop`` / ``thot gui``)
     Built inside the agent checkout under ``$THOT_HOME/thot-agent/``:
       ...
       - ``$THOT_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Thot.app`` or ``~/Applications/Thot.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Thot`` (NSIS per-user)
...
the app name ("Thot"), separate from ``$THOT_HOME``:
  - macOS:   ``~/Library/Application Support/Thot``
  - Windows: ``%APPDATA%\\Thot``
  - Linux:   ``$XDG_CONFIG_HOME/Thot`` (default ``~/.config/Thot``)
...
``thot uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Thot"
...
def source_built_gui_artifacts(thot_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``thot desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``thot-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(thot_home)
    ...
    return [
        ...
        thot_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Thot" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Thot).
            local_base / "Programs" / "Thot",
            # Older / alternate layout some builds used.
            local_base / "thot-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Thot")
    else:
        ...
        paths += [
            data_base / "applications" / "thot.desktop",
            data_base / "applications" / "Thot.desktop",
        ]
    return paths


def agent_is_installed(thot_home: Path) -> bool:
    """Return True when a usable Python agent install exists under THOT_HOME.
...
def gui_is_installed(thot_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(thot_home):
        if p.exists():
            return True
...
def gui_install_summary(thot_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = thot_home if thot_home is not None else get_thot_home()
...
def uninstall_gui(thot_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``thot-agent/thot_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$THOT_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove thot' or "
            "'sudo dnf remove thot'). AppImage builds are a single file you "

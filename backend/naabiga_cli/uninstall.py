"""
Naabiga Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.naabiga/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Naabiga PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing naabiga-agent or naabiga PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Naabiga Agent" comment and following line
                if '# Naabiga Agent' in line or '# naabiga-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('naabiga' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing naabiga
                if 'naabiga' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the naabiga wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "naabiga",
        Path("/usr/local/bin/naabiga"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains naabiga_cli reference)
                content = wrapper.read_text()
                if 'naabiga_cli' in content or 'naabiga-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(naabiga_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``naabiga`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Naabiga home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (naabiga_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``naabiga gateway run`` processes
    ...
    try:
        from naabiga_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from naabiga_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NAABIGA_HOME`` and ``NAABIGA_GIT_BASH_PATH``
...
def _naabiga_path_markers(naabiga_home: Path) -> list[str]:
    """Path-entry substrings that identify Naabiga-owned User-PATH entries."""
    root = str(naabiga_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare naabiga-agent install dir.
    markers = [root + "\\naabiga-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if NAABIGA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "naabiga".  We don't want to catch
    # unrelated entries like "cnaabiga-foo" or "ephermeral", so we look for
    # backslash-naabiga as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(naabiga_home: Path) -> list[str]:
    """Strip Naabiga-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _naabiga_path_markers(naabiga_home)
            kept: list[str] = []
...
def remove_naabiga_env_vars_windows() -> list[str]:
    """Delete NAABIGA_HOME and NAABIGA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NAABIGA_HOME", "NAABIGA_GIT_BASH_PATH"):
                try:
...
def _is_default_naabiga_home(naabiga_home: Path) -> bool:
    """Return True when ``naabiga_home`` points at the default (non-profile) root."""
    try:
        from naabiga_constants import get_default_naabiga_root
        return naabiga_home.resolve() == get_default_naabiga_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NAABIGA_HOME directory.

    We shell out to ``naabiga -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NAABIGA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m naabiga_cli.main` so we don't depend on a `naabiga`
    #    wrapper that may be half-removed mid-uninstall.
    naabiga_invocation = [_sys.executable, "-m", "naabiga_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                naabiga_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's NAABIGA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``naabiga uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$NAABIGA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from naabiga_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(naabiga_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Naabiga Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Naabiga Chat GUI installation was found.")
        print(f"  Checked: {naabiga_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Naabiga agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(naabiga_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Naabiga agent at {naabiga_home / 'naabiga-agent'}")
        print(f"  • Your config, sessions, and secrets under {naabiga_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Naabiga agent is still installed. Run 'naabiga' to use the CLI,")
    print("or 'naabiga uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.naabiga/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.naabiga/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie NAABIGA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_naabiga_home(naabiga_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.naabiga data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Naabiga Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {naabiga_home / 'config.yaml'}")
    print(f"  Secrets: {naabiga_home / '.env'}")
    print(f"  Data:    {naabiga_home / 'cron/'}, {naabiga_home / 'sessions/'}, {naabiga_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their NAABIGA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Naabiga data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Naabiga code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, naabiga_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Naabiga PATH entries from shell configs / Windows User PATH")
    print("  • Naabiga wrapper scripts and Naabiga-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Naabiga config/data: {naabiga_home}")
        if _is_default_naabiga_home(naabiga_home):
        ...
        print(f"  • Keep Naabiga config/data: {naabiga_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``naabiga`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$NAABIGA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in naabiga_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\naabiga\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(naabiga_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Naabiga-owned PATH entries in User environment")

        log_info("Removing NAABIGA_HOME / NAABIGA_GIT_BASH_PATH User env vars...")
        removed_env = remove_naabiga_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Naabiga-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing naabiga command...")
    ...
    #     (only when they still point into this Naabiga home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Naabiga-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(naabiga_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Naabiga-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(naabiga_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE NAABIGA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from naabiga_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(naabiga_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.naabiga/, just remove the naabiga-agent subdir
            if naabiga_home in project_root.parents or project_root.parent == naabiga_home:
    ...
    #     under NAABIGA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(naabiga_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(naabiga_home)
    ...
    # 5. Optionally remove ~/.naabiga/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NAABIGA_HOME dirs live under
        ...
        try:
            if naabiga_home.exists():
                shutil.rmtree(naabiga_home)
                log_success(f"Removed {naabiga_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {naabiga_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {naabiga_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {naabiga_home}/")
        print()
        print("To reinstall later with your existing settings:")
        print(color("  See the repo README: https://github.com/n8nprobf-hub/Naabiga#readme", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Naabiga Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m naabiga_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import naabiga_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``naabiga_constants`` + ``naabiga_cli.colors``
    (and lazily ``naabiga_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m naabiga_cli.uninstall")
    parser.add_argument(
        "--mode",

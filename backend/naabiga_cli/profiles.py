"""
Profile management for multiple isolated Naabiga instances.

Each profile is a fully independent NAABIGA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.naabiga/profiles/<name>/`` by default.

The "default" profile is ``~/.naabiga`` itself — backward compatible,
zero migration needed.

Usage::

    naabiga profile create coder          # fresh profile + bundled skills
    naabiga profile create coder --clone  # also copy config, .env, SOUL.md, skills
    naabiga profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    naabiga -p coder chat                 # or via flag
    naabiga profile use coder             # set as sticky default
    naabiga profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See naabiga_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.naabiga``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   naabiga-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `naabiga backup` archives
...
# Marker file written by `naabiga profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `naabiga
...
# `naabiga skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Naabiga infrastructure directories that only the default profile
         (``~/.naabiga``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_naabiga_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.naabiga) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "naabiga-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when NAABIGA_HOME equals the
...
# into the export. The set below identifies the *known Naabiga profile
# artifacts* at the root of NAABIGA_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``naabiga_constants``.
...
_RESERVED_NAMES = frozenset({
    "naabiga", "default", "test", "tmp", "root", "sudo",
})

# Naabiga subcommands that cannot be used as profile names/aliases
_NAABIGA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the naabiga root, NOT to the current NAABIGA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NAABIGA_HOME points outside
    ``~/.naabiga``, profiles live under ``NAABIGA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_naabiga_home() / "profiles"


def _get_default_naabiga_home() -> Path:
    """Return the default (pre-profile) NAABIGA_HOME path.

    In standard deployments this is ``~/.naabiga``.
    In Docker/custom deployments where NAABIGA_HOME is outside ``~/.naabiga``
    (e.g. ``/opt/data``), returns NAABIGA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_naabiga_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Naabiga installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NAABIGA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_naabiga_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, naabiga subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _NAABIGA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a naabiga subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            naabiga_exe = shutil.which("naabiga") or "naabiga"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(naabiga_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Naabiga. If we
    ...
    try:
        ...
        from naabiga_cli.config import check_config_version, migrate_config

        token = set_naabiga_home_override(str(profile_dir))
        try:
        ...
        # not be migrated. The next `naabiga doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``naabiga -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``naabiga profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``naabiga -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``naabiga profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``NAABIGA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Naabiga config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_naabiga_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, naabiga_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own NAABIGA_HOME.
    ...
    The returned ``naabiga_home`` is the path to pass to
    ``set_naabiga_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_naabiga_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``naabiga update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.naabiga)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.naabiga/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `naabiga -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Naabiga profile.\n"
    ...
    if not soul_path.exists():
        try:
            from naabiga_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `naabiga update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`naabiga profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `naabiga update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Naabiga (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `naabiga profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `naabiga -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NAABIGA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``naabiga profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NAABIGA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_naabiga_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Naabiga *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Naabiga backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``NAABIGA_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `naabiga -p <canon> profile
    ...
    naabiga_markers = ("naabiga_cli.main", "naabiga-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Naabiga process: either an entrypoint marker in argv, or
            # a resolved executable named `naabiga`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_naabiga = (
                any(marker in joined for marker in naabiga_markers)
                or exe_name == "naabiga"
                or exe_name.startswith("naabiga")
            )
            if not is_naabiga:
                continue
            ...
            # ...or by NAABIGA_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("NAABIGA_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.naabiga).\n"
            "To remove everything, use: naabiga uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_naabiga_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``NAABIGA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from naabiga_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NAABIGA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NAABIGA_HOME")
    try:
        os.environ["NAABIGA_HOME"] = str(profile_dir)
        from naabiga_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NAABIGA_HOME"] = old_home
        elif "NAABIGA_HOME" in os.environ:
            del os.environ["NAABIGA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.naabiga/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: naabiga profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NAABIGA_HOME.

    Returns ``"default"`` if NAABIGA_HOME is not set or points to ``~/.naabiga``.
    Returns the profile name if NAABIGA_HOME points into ``~/.naabiga/profiles/<name>``.
    Returns ``"custom"`` if NAABIGA_HOME is set to an unrecognized path.
    """
    from naabiga_constants import get_naabiga_home
    naabiga_home = get_naabiga_home()
    resolved = naabiga_home.resolve()

    default_resolved = _get_default_naabiga_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      NAABIGA_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Naabiga file the
      user may have lying alongside NAABIGA_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.naabiga itself — its parent is ~/ and its
        # directory name is ".naabiga", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: naabiga profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.naabiga itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.naabiga). "
            "Specify a different name: naabiga profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="naabiga_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"naabiga_{old_name}"
    legacy_old_host = f"naabiga.{old_name}"
    new_host = f"naabiga_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_naabiga_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("naabiga_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NAABIGA_HOME path string.

    Called early in the CLI entry point, before any naabiga modules
    are imported, to set the NAABIGA_HOME environment variable.
    """

"""
Profile management for multiple isolated Thot instances.

Each profile is a fully independent THOT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.thot/profiles/<name>/`` by default.

The "default" profile is ``~/.thot`` itself — backward compatible,
zero migration needed.

Usage::

    thot profile create coder          # fresh profile + bundled skills
    thot profile create coder --clone  # also copy config, .env, SOUL.md, skills
    thot profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    thot -p coder chat                 # or via flag
    thot profile use coder             # set as sticky default
    thot profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See thot_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `thot profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `thot
...
# `thot skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Thot infrastructure directories that only the default profile
         (``~/.thot``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_thot_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.thot) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "thot-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when THOT_HOME equals the
...
# into the export. The set below identifies the *known Thot profile
# artifacts* at the root of THOT_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``thot_constants``.
...
_RESERVED_NAMES = frozenset({
    "thot", "default", "test", "tmp", "root", "sudo",
})

# Thot subcommands that cannot be used as profile names/aliases
_THOT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the thot root, NOT to the current THOT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where THOT_HOME points outside
    ``~/.thot``, profiles live under ``THOT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_thot_home() / "profiles"


def _get_default_thot_home() -> Path:
    """Return the default (pre-profile) THOT_HOME path.

    In standard deployments this is ``~/.thot``.
    In Docker/custom deployments where THOT_HOME is outside ``~/.thot``
    (e.g. ``/opt/data``), returns THOT_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_thot_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Thot installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its THOT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_thot_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, thot subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _THOT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a thot subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            thot_exe = shutil.which("thot") or "thot"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(thot_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Thot. If we
    ...
    try:
        ...
        from thot_cli.config import check_config_version, migrate_config

        token = set_thot_home_override(str(profile_dir))
        try:
        ...
        # not be migrated. The next `thot doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``thot -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``thot profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``thot -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``thot profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``THOT_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Thot config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_thot_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, thot_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own THOT_HOME.
    ...
    The returned ``thot_home`` is the path to pass to
    ``set_thot_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_thot_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``thot update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.thot)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.thot/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `thot -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Thot profile.\n"
    ...
    if not soul_path.exists():
        try:
            from thot_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `thot update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`thot profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `thot update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Thot (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `thot profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `thot -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches THOT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``thot profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "THOT_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_thot_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Thot *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Thot backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``THOT_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `thot -p <canon> profile
    ...
    thot_markers = ("thot_cli.main", "thot-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Thot process: either an entrypoint marker in argv, or
            # a resolved executable named `thot`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_thot = (
                any(marker in joined for marker in thot_markers)
                or exe_name == "thot"
                or exe_name.startswith("thot")
            )
            if not is_thot:
                continue
            ...
            # ...or by THOT_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("THOT_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.thot).\n"
            "To remove everything, use: thot uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_thot_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``THOT_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from thot_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set THOT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("THOT_HOME")
    try:
        os.environ["THOT_HOME"] = str(profile_dir)
        from thot_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["THOT_HOME"] = old_home
        elif "THOT_HOME" in os.environ:
            del os.environ["THOT_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.thot/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: thot profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from THOT_HOME.

    Returns ``"default"`` if THOT_HOME is not set or points to ``~/.thot``.
    Returns the profile name if THOT_HOME points into ``~/.thot/profiles/<name>``.
    Returns ``"custom"`` if THOT_HOME is set to an unrecognized path.
    """
    from thot_constants import get_thot_home
    thot_home = get_thot_home()
    resolved = thot_home.resolve()

    default_resolved = _get_default_thot_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      THOT_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Thot file the
      user may have lying alongside THOT_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.thot itself — its parent is ~/ and its
        # directory name is ".thot", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: thot profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.thot itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.thot). "
            "Specify a different name: thot profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="thot_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"thot_{old_name}"
    legacy_old_host = f"thot.{old_name}"
    new_host = f"thot_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_thot_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("thot_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a THOT_HOME path string.

    Called early in the CLI entry point, before any thot modules
    are imported, to set the THOT_HOME environment variable.
    """

"""``naabiga slack ...`` CLI subcommands.

Today only ``naabiga slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ naabiga slack manifest > slack-manifest.json
    # or:
    $ naabiga slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Naabiga. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Naabiga deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from naabiga_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Naabiga in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Naabiga agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``naabiga_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$NAABIGA_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Naabiga")
    ...
    """
    name = getattr(args, "name", None) or "Naabiga"
    description = getattr(args, "description", None) or "Your Naabiga agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("NAABIGA_HOME") or str(Path.home() / ".naabiga")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Naabiga app\n"

"""Profile distributions — shareable, packaged Naabiga profiles via git.

A distribution is a Naabiga profile published as a git repository (or
...
* ``naabiga profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``naabiga skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``naabiga profile``, not a parallel tree):

    naabiga profile install <source> [--name N] [--alias] [--force] [--yes]
    naabiga profile update  <name>  [--force-config] [--yes]
    naabiga profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "naabiga_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".naabiga_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "naabiga-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            naabiga_requires=str(data.get("naabiga_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.naabiga_requires:
            out["naabiga_requires"] = self.naabiga_requires
        if self.author:
...
def check_naabiga_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Naabiga {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Naabiga distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``naabiga profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Naabiga profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Naabiga distribution."
        )

    # Version check up-front so we fail fast
    check_naabiga_requires(manifest.naabiga_requires, naabiga_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.naabiga).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="naabiga_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `naabiga profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`naabiga profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="naabiga_dist_update_") as tmp:
        plan = plan_install(

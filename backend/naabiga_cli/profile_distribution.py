"""Profile distributions — shareable, packaged Thot profiles via git.

A distribution is a Thot profile published as a git repository (or
...
* ``thot profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``thot skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``thot profile``, not a parallel tree):

    thot profile install <source> [--name N] [--alias] [--force] [--yes]
    thot profile update  <name>  [--force-config] [--yes]
    thot profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "thot-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            thot_requires=str(data.get("thot_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.thot_requires:
            out["thot_requires"] = self.thot_requires
        if self.author:
...
def check_thot_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Thot {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Thot distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``thot profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Thot profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Thot distribution."
        )

    # Version check up-front so we fail fast
    check_thot_requires(manifest.thot_requires, thot_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.thot).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="thot_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `thot profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`thot profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="thot_dist_update_") as tmp:
        plan = plan_install(

"""Slash command definitions and autocomplete for the Naabiga CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("start", "Accuser réception d'un ping de démarrage de plateforme sans répondre", "Session",
               gateway_only=True),
    CommandDef("new", "Démarrer une nouvelle session (nouvel ID de session + historique)", "Session",
               aliases=("reset",), args_hint="[name]"),
    CommandDef("topic", "Activer ou inspecter les sessions par sujet Telegram (DM)", "Session",
               gateway_only=True, args_hint="[off|help|session-id]"),
    CommandDef("clear", "Effacer l'écran et démarrer une nouvelle session", "Session",
               cli_only=True),
    CommandDef("redraw", "Forcer un rafraîchissement complet de l'interface (répare le décalage du terminal)", "Session",
               cli_only=True),
    ...
    CommandDef("save", "Sauvegarder la conversation en cours", "Session",
               cli_only=True),
    CommandDef("retry", "Renvoyer le dernier message (le renvoyer à l'agent)", "Session"),
    CommandDef("prompt", "Rédiger votre prochain prompt dans $EDITOR (markdown), puis l'envoyer", "Session",
               cli_only=True, args_hint="[initial text]", aliases=("compose",)),
    CommandDef("undo", "Reculer de N tours utilisateur et relancer le prompt (défaut 1)", "Session",
               args_hint="[N]"),
    CommandDef("title", "Définir un titre pour la session en cours", "Session",
               args_hint="[name]"),
    CommandDef("handoff", "Transférer cette session vers une plateforme de messagerie (Telegram, Discord, etc.)", "Session",
               args_hint="<platform>", cli_only=True),
    CommandDef("branch", "Brancher la session en cours (explorer un chemin différent)", "Session",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("compress", "Compresser le contexte de conversation ('here [N]' conserve les N derniers tours ; --preview montre l'effet)", "Session",
               aliases=("compact",), args_hint="[here [N] | focus topic | --preview|--dry-run]"),
    CommandDef("rollback", "Lister ou restaurer des points de restauration du système de fichiers", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Créer ou restaurer des instantanés d'état de la config/état de Naabiga", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Tuer tous les processus d'arrière-plan en cours", "Session"),
    CommandDef("approve", "Approuver une commande dangereuse en attente", "Session",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "Refuser une commande dangereuse en attente (éventuellement avec une raison)", "Session",
    ...
    CommandDef("agents", "Afficher les agents actifs et les tâches en cours", "Session",
               aliases=("tasks",)),
    CommandDef("journey", "Ouvrir la chronologie d'apprentissage",
               "Session", aliases=("learning", "memory-graph"), cli_only=True,
               args_hint="[list|delete <id>|edit <id>]",
               subcommands=("list", "delete", "edit")),
    CommandDef("queue", "Mettre un prompt en file pour le prochain tour (n'interrompt pas)", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "Injecter un message après le prochain appel d'outil sans interrompre", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Définir un objectif permanent sur lequel Naabiga travaille jusqu'à réalisation", "Session",
               args_hint="[text | draft <text> | show | pause | resume | clear | status | wait <pid> | unwait]"),
    CommandDef("moa", "Exécuter un prompt via le preset Mixture of Agents par défaut, puis restaurer votre modèle", "Session",
               args_hint="<prompt>"),
    CommandDef("subgoal", "Ajouter ou gérer des critères supplémentaires sur l'objectif actif", "Session",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "Afficher les infos de session, modèle, jetons et contexte", "Session"),
    CommandDef("whoami", "Afficher votre accès aux commandes slash (admin / utilisateur)", "Info"),
    CommandDef("profile", "Afficher le nom du profil actif et son répertoire d'accueil", "Info"),
    CommandDef("sethome", "Définir ce chat comme canal d'accueil", "Session",
    ...
    CommandDef("config", "Afficher la configuration actuelle", "Configuration",
               cli_only=True),
    CommandDef("model", "Changer de modèle (persistant par défaut)", "Configuration",
               args_hint="[model] [--provider name] [--global|--session] [--refresh]"),
    CommandDef("codex-runtime", "Activer/désactiver le runtime serveur d'app codex pour les modèles OpenAI/Codex",
               "Configuration", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]"),

    CommandDef("personality", "Définir une personnalité prédéfinie", "Configuration",
               args_hint="[name]"),
    CommandDef("statusbar", "Activer/désactiver la barre d'état contexte/modèle", "Configuration",
               cli_only=True, aliases=("sb",)),
    CommandDef("timestamps", "Activer/désactiver les horodatages [HH:MM] sur les messages et /history", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status"), aliases=("ts",)),
    CommandDef("verbose", "Faire défiler l'affichage de progression des outils : off -> new -> all -> verbose -> log",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "Activer/désactiver le pied de page des métadonnées d'exécution sur les réponses finales",
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "Activer/désactiver le mode YOLO (saute toutes les approbations de commandes dangereuses)",
               "Configuration"),
    ...
    CommandDef("fast", "Activer/désactiver le mode rapide — priorité de traitement OpenAI / mode rapide Anthropic (Normal/Fast)", "Configuration",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "Afficher ou changer le thème/skin d'affichage", "Configuration",
               cli_only=True, args_hint="[name]"),
    CommandDef("indicator", "Choisir le style d'indicateur activité de l'interface", "Configuration",
    ...
    CommandDef("busy", "Contrôler ce que fait Entrée pendant que Naabiga travaille", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "Gérer les outils : /tools [list|disable|enable] [name...]", "Tools & Skills",
    ...
    CommandDef("memory", "Réviser les écritures de mémoire en attente / basculer la porte d'approbation",
               "Tools & Skills",
               args_hint="[pending|approve|reject|approval] [id|on|off]",
               subcommands=("pending", "approve", "reject", "approval")),
    CommandDef("bundles", "Lister les groupes de compétences (aliases /<name> pour plusieurs compétences)",
               "Tools & Skills"),
    CommandDef("pet", "Basculer ou adopter une mascotte petdex (/pet, /pet list, /pet <slug>)", "Tools & Skills",
               cli_only=True, args_hint="[toggle|list|scale <n>|<slug>]", subcommands=("toggle", "list", "scale", "off")),
    CommandDef("hatch", "Générer une nouvelle mascotte petdex à partir d'une description",
               "Tools & Skills", cli_only=True, aliases=("generate-pet",), args_hint="[description]"),
    CommandDef("learn", "Apprendre une compétence réutilisable à partir de ce que vous décrivez (dossiers, URL, ce chat, notes)",
               "Tools & Skills", args_hint="<what to learn from>"),
    CommandDef("cron", "Gérer les tâches planifiées", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("suggestions", "Examiner les automations suggérées (accepter/ignorer)",
               "Tools & Skills", aliases=("suggest",), args_hint="[accept|dismiss N | catalog]",
               subcommands=("accept", "dismiss", "catalog", "clear")),
    CommandDef("blueprint", "Configurer une automation à partir d'un modèle",
               "Tools & Skills", aliases=("bp",), args_hint="[name] [slot=value ...]"),
    CommandDef("curator", "Maintenance des compétences en arrière-plan (état, exécution, épinglage, archivage)",
    ...
    CommandDef("reload-mcp", "Recharger les serveurs MCP depuis la configuration", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scanner ~/.naabiga/skills/ pour les compétences récemment installées ou supprimées",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "Connecter les outils navigateur à votre navigateur Chromium en direct via CDP", "Tools & Skills",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "Lister les plugins installés et leur état",
               "Tools & Skills", cli_only=True),
    ...
    CommandDef("restart", "Redémarrer proprement la passerelle après avoir vidé les exécutions actives", "Session",
               gateway_only=True),
    CommandDef("usage", "Afficher l'utilisation de jetons et les limites de débit de la session actuelle", "Info"),
    CommandDef("credits", "Afficher le solde de crédits Nous et recharger", "Info"),
    CommandDef("billing", "Gérer la facturation terminale Nous — acheter des crédits, rechargement auto, limites", "Info",
               cli_only=True),
    CommandDef("insights", "Afficher les informations et analyses d'utilisation", "Info",
               args_hint="[days]"),
    CommandDef("platforms", "Afficher l'état de la passerelle / des plateformes de messagerie", "Info",
               cli_only=True, aliases=("gateway",)),
    CommandDef("platform", "Mettre en pause, reprendre ou lister une plateforme de passerelle défaillante", "Info",
    ...
    CommandDef("paste", "Joindre l'image du presse-papiers depuis votre presse-papiers", "Info",
               cli_only=True),
    CommandDef("image", "Joindre un fichier image local pour votre prochain prompt", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "Mettre à jour Naabiga Agent vers la dernière version", "Info"),
    CommandDef("version", "Afficher la version de Naabiga Agent", "Info", aliases=("v",)),
    ...
    # Exit
    CommandDef("quit", "Quitter le CLI (--delete supprime aussi l'historique de session)", "Exit",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`naabiga_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/naabiga`` subcommand mapping, and
    ...
    """
    try:
        from naabiga_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. Naabiga ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Naabiga
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``naabiga skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``naabiga skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``naabiga skills list`` and
...
# unrelated command landed. These claim their slots right after /naabiga,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /naabiga <command>).
...
# native slot, the alias spelling stays reachable via /naabiga reset).
...
# ``/naabiga <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/naabiga" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /naabiga credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /naabiga billing.
#   - moa: high-cost slash mode, available through /naabiga moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /naabiga debug on Slack.
_SLACK_VIA_NAABIGA_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/naabiga <verb>`` subcommand.
    ...
    can still reach them via ``/naabiga <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/naabiga`` is always reserved as the first entry so the
    legacy ``/naabiga <subcommand>`` form keeps working for anything that
    ...
    # Reserve /naabiga as the catch-all top-level command.
    entries.append(("naabiga", "Talk to Naabiga or run a subcommand", "[subcommand] [args]"))
    seen.add("naabiga")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_NAABIGA_ONLY:
            # Intentionally Slack-via-/naabiga only (see _SLACK_VIA_NAABIGA_ONLY).
            return
    ...
    # everything except /naabiga, so a new canonical command can never silently
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /naabiga handler.

    Maps both canonical names and aliases so /naabiga bg do stuff works
    the same as /naabiga background do stuff.

    Plugin-registered slash commands are included so ``/naabiga <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from naabiga_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _tools_completions(sub_text: str, sub_lower: str):
        ...
        try:
            ...
            from naabiga_cli.tools_config import (

...
def _get_custom_provider_names() -> list:
    ...
    try:
        from thot_cli.config import get_compatible_custom_providers, load_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_thot_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:thot_pkce",
    ...
    if provider == "nous":
        ...
        # <thot-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `thot --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``thot auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`thot auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Thot reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `thot auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `thot auth` is called bare."""

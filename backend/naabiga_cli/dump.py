"""
Dump command for naabiga CLI.

Outputs a compact, plain-text summary of the user's Naabiga setup
...
from naabiga_cli.config import get_naabiga_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.naabiga/.env.
    ...
    exports. ``naabiga debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.naabiga_build_sha`` by the Dockerfile's
    ``NAABIGA_GIT_SHA`` build-arg (see ``naabiga_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``naabiga dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from naabiga_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(naabiga_home: Path) -> int:
...
def _cron_summary(naabiga_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = naabiga_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"naabiga_home:      {display_naabiga_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.naabiga/.env: a managed
        ...
        # A credential added via `naabiga auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `naabiga auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["naabiga-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(naabiga_home)}")
    lines.append(f"  skills:             {_count_skills(naabiga_home)}")

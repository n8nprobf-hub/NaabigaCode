"""
Dump command for thot CLI.

Outputs a compact, plain-text summary of the user's Thot setup
...
from thot_cli.config import get_thot_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.thot/.env.
    ...
    exports. ``thot debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.thot_build_sha`` by the Dockerfile's
    ``THOT_GIT_SHA`` build-arg (see ``thot_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``thot dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from thot_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(thot_home: Path) -> int:
...
def _cron_summary(thot_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = thot_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"thot_home:      {display_thot_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.thot/.env: a managed
        ...
        # A credential added via `thot auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `thot auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["thot-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(thot_home)}")
    lines.append(f"  skills:             {_count_skills(thot_home)}")

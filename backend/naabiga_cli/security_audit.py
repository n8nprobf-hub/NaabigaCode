"""On-demand supply-chain audit for Naabiga Agent installs.

Scans three surfaces a Naabiga user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Naabiga venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.naabiga/plugins``
...
def _discover_plugins(naabiga_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.naabiga/plugins``.
    ...
    """
    plugins_dir = naabiga_home / "plugins"
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from naabiga_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def run_audit(
    *,
    ...
    home = naabiga_home or Path(get_naabiga_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, naabiga_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(naabiga_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `naabiga security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, naabiga_home=home
    )
    if total == 0:

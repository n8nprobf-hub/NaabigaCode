"""MCP picker — interactive `naabiga mcp picker` (also the default `naabiga mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``naabiga mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `naabiga plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from naabiga_cli.cli_output import prompt_yes_no
from naabiga_cli.curses_ui import curses_single_select
from naabiga_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Naabiga session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from naabiga_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `naabiga mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: naabiga mcp install <name>    Picker: naabiga mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Naabiga is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Naabiga — run `naabiga update` "
...
def show_catalog() -> None:
    """`naabiga mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`naabiga mcp picker` (and default `naabiga mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`naabiga mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from naabiga_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `naabiga mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
